********************************************************************************
* UNION SPILLOVERS — LAYER-LEVEL SPILLOVER BY FIRM SIZE (ZERO-FILLED EMPLOYMENT)
*
* Mirrors 07_layer_spillover_size.do but uses the zero-filled outcome data
* (firm_layer_outcomes_{layer}_full.dta) produced by 06b_prep_layer_outcomes.py
* and 06b_prep_demog_outcomes.py.
*
* Key differences from 07_layer_spillover_size.do:
*   - Loads *_full.dta: zero-employment cells are included for all layers.
*   - Employment outcome: l1p_layer_emp = ln(1 + layer_emp) (valid at zero).
*   - Absorb uses l1p_layer_emp_pre4 instead of l_layer_emp_pre4.
*   - Connectivity set to zero for firms that never had flows through that layer
*     (layer_treat_pw_n = 0 if missing after merge).
*   - firmrestr uses layer_had_flows flag (saved before zero-fill) to preserve
*     original restriction: firm-level spec runs only on firms active in the layer.
*
...
*
* Output:
*   Tables/layer_connectivity/results_spill_layer_{layer}_size_{size}_full_layer_spill.csv
*   Tables/layer_connectivity/results_spill_firmrestr_{layer}_size_{size}_full_layer_spill.csv
*   Graphs/layer_connectivity/es_{outcome}_spill_{layer}_size_{size}_full_{date}.pdf
********************************************************************************
...
log using "$logs/layer_connectivity/layer_spillover_size_full_`d'_`t'.log", replace text

di "Started: `c(current_date)' `c(current_time)'"
di "Stata version: `c(stata_version)'"

local spec "full_layer_spill"
...
foreach layer in edu edu2 gender race {
	...
	* ── 2a. Load outcomes (firm × layer × year, zero-employment cells included) ──
	use "$layer_data/firm_layer_outcomes_`layer'_full.dta", clear
	keep if year >= 2009
	...
	* Flag firms that had actual layer flows (before zero-filling).
	* Used in firmrestr section to preserve the original restriction.
	gen byte layer_had_flows = !missing(layer_treat_pw_n)

	* Zero-fill connectivity: firms with no pre-treatment flows through this layer
	* get connectivity = 0 rather than being dropped from regressions.
	replace layer_treat_pw_n = 0 if missing(layer_treat_pw_n)
	...
	* l1p_layer_emp included; l_layer_emp kept for wage regression controls.
	foreach v in lr_remdezr_layer lr_remdezr_h_layer l_layer_emp l1p_layer_emp {
		cap drop `v'_pre_o
		cap drop `v'_pre
	...
	****************************************************************************
	...
	foreach size in small large {
		...
		* Employment outcome l1p_layer_emp replaces l_layer_emp (valid at zero).
		* Wage outcomes unchanged; their absorb uses l1p_layer_emp_pre4 as the
		* employment control (virtually identical to l_layer_emp_pre4 for
		* positive-employment cells, which is all wage-regression observations).
		foreach outcome in lr_remdezr_layer lr_remdezr_h_layer l1p_layer_emp {

			di as text "  Estimating: `outcome' (`layer', `size')"

			local absorb ///
				"`base_fe' ib0.`outcome'_pre4#i.year ib0.l1p_layer_emp_pre4#i.year `extra_year'"
			...
			* Event study plot
			...
			graph export "$graphs/layer_connectivity/es_`outcome'_spill_`layer'_size_`size'_full_`d'.pdf", ///
				as(pdf) replace
		...
		************************************************************************
		...
		foreach outcome in lr_remdezr_layer lr_remdezr_h_layer l1p_layer_emp {
			...
			local absorb_cross ///
				"`base_fe_cross' ib0.`outcome'_pre4#i.year ib0.l1p_layer_emp_pre4#i.year `extra_year'"
			...
			* Event study plot
			...
			graph export "$graphs/layer_connectivity/es_`outcome'_cross_`layer'_size_`size'_full_`d'.pdf", ///
				as(pdf) replace
	...
	*
	* NOTE: uses layer_had_flows (flagged before zero-fill) to restrict to firms
	* that actually had pre-treatment flows through this layer — same intent as
	* the original !missing(layer_treat_pw_n) filter.
	****************************************************************************
	...
	* ── Phase 1: Extract restricted firm sets by size ────────────────────
	...
	foreach size in small large {
		...
		preserve
		keep if (`s_spill') & layer_had_flows & size_large == `size_ind'
		keep identificad
		duplicates drop
		count
...
di _newline(2)
di as result "======================================================================="
di as result "07_layer_spillover_size_full.do complete."
di as result "Output CSVs: $tables/layer_connectivity/results_spill_layer_*_size_*_full*"
di as result "======================================================================="

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Stata done" "07_layer_spillover_size_full: all layers × size groups complete"

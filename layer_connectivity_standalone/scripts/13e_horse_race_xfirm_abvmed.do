********************************************************************************
* UNION SPILLOVERS — HORSE RACE (CROSS-FIRM FE, ABOVE-MEDIAN BOTH LAYERS)
* Purpose: Same as 13b_horse_race_edu2_xfirm.do but generalised to all three
*          layer definitions (edu2, gender, race) and restricted to firms where
...
*   Tables/layer_connectivity/results_horse_race_{edu2,gender,race}_xfirm_abvmed.csv
********************************************************************************
...
log using "$logs/layer_connectivity/horse_race_xfirm_abvmed_`d'_`t'.log", replace text
...
foreach layer in edu2 gender race {
	...
	* ── Layer-specific split labels and above-median thresholds ──────────────
	if "`layer'" == "edu2" {
		local lv1      "no_hs"
		local lv2      "has_hs"
		local med_lv1  = 10.0
	...
	* Scale layer connectivity to P90 of untreated balanced-panel above-median firms at 2009
	local s_spill "lagos_sample_avg==1 & treat_ultra==0 & in_balanced_panel==1 & in_above_med_both==1"
	...
	****************************************************************************
	...
	local csv_out "$tables/layer_connectivity/results_horse_race_`layer'_xfirm_abvmed.csv"
		...
		foreach lv_o in `lv1' `lv2' {
			...
			* ── Post-treatment DiD ───────────────────────────────────────────
			reghdfe `outcome' c.c_`lv1'##i.treat_year c.c_`lv2'##i.treat_year ///
				if treat_ultra==0 & in_balanced_panel==1 & in_above_med_both==1 & layer_id=="`lv_o'", ///
			...
			* ── Pre-period placebo ───────────────────────────────────────────
			reghdfe `outcome' c.c_`lv1'##i.placebo_year c.c_`lv2'##i.placebo_year ///
				if treat_ultra==0 & in_balanced_panel==1 & in_above_med_both==1 & year<=2011 & layer_id=="`lv_o'", ///
				absorb(`absorb') vce(cluster identificad)
			...
			* ── Write to CSV ──────────────────────────────────────────────────
			local spec_key "horse_race_`layer'_xfirm_abvmed"
			tempname fh
	...
	****************************************************************************
	...
	preserve
		...
		* Collapse to firm×year — c_lv*, in_above_med_both are firm-constant
		collapse (first) c_`lv1' c_`lv2' treat_ultra in_balanced_panel in_above_med_both ///
		...
		* Merge pre-treatment totalflows
		...
		local s_spill_f "treat_ultra==0 & in_balanced_panel==1 & in_above_med_both==1"
...
di as result "Horse race (cross-firm FE, above-median) complete."

log close

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Horse race xfirm abvmed done" "13e: all layers complete"

********************************************************************************
* 05_run_within_firm_hw.do
* Within-firm group-level exhibits (A6/A7/A8) on CORRECTED LOG HOURLY WAGES.
* Produces the estimates behind Tables 22, 23 and 24.
*
* Uses the CURRENT recomputable firm connectivity measure.
*
* Run from Stata:   do "scripts/05_run_within_firm_hw.do"
* Run from shell:   stata-mp -b do scripts/05_run_within_firm_hw.do
*
...
* Strip /scripts/05_run_within_firm_hw.do
local root = subinstr(`"`this'"', "/scripts/05_run_within_firm_hw.do", "", 1)
...
capture log close within_firm_hw_log
local d = subinstr("`c(current_date)'"," ","_",.)
local t = subinstr("`c(current_time)'",":","",.)
log using "$output/within_firm_hw_`d'_`t'.log", replace text name(within_firm_hw_log)

di as result "--- Within-firm exhibits (A6/A7/A8): log hourly wages (corrected) ---"
di as result "Started: `c(current_date)' `c(current_time)'"

********************************************************************************
* HOURLY PASS -> a6_group_hw.csv, a6_partition_hw.csv, a7_hw.csv, a8_hw.csv
********************************************************************************

global wf_wage_firm  "lr_remdezr_h_w"
global wf_wage_layer "lr_remdezr_h_layer"
global wf_suffix     "_hw"

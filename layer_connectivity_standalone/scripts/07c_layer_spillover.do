********************************************************************************
* UNION SPILLOVERS — LAYER-LEVEL SPILLOVER EFFECTS (ABOVE-MEDIAN BOTH LAYERS)
* Purpose: Same as 07b_layer_spillover.do but restricted to firms where BOTH
*          layers have average pre-treatment employment (2009-2011) above the
*          population median for that split.
*
*   Hardcoded medians (from firm_layer_outcomes data, 2009-2011):
*     edu2:   no_hs  > 10.0,    has_hs > 19.667
*     gender: female > 10.0,    male   > 16.0
*     race:   nonwhite > 7.333, white  > 18.0
*
...
*
* Output:  Tables/layer_connectivity/results_spill_layer_{edu2,gender,race}_abvmed_layer_spill.csv
*          Tables/layer_connectivity/results_spill_layer_cross_{edu2,gender,race}_abvmed_layer_spill.csv
*          Tables/layer_connectivity/results_spill_firmrestr_{edu2,gender,race}_abvmed_layer_spill.csv
*          Graphs/layer_connectivity/es_{outcome}_abvmed_{spill,cross,firmrestr}_{layer}_{date}.pdf
********************************************************************************
...
log using "$logs/layer_connectivity/layer_spillover_abvmed_`d'_`t'.log", replace text

di "Started: `c(current_date)' `c(current_time)'"
di "Stata version: `c(stata_version)'"

global layer_data "$main/UnionSpill/Data/layer_connectivity"
local  spec       "abvmed_layer_spill"
...
foreach layer in edu2 gender race {
	...
	* Medians of avg pre-treatment (2009-2011) employment, computed across all
	* firms with both layers present. Hardcoded from population-level computation.
	if "`layer'" == "edu2" {
		local lv1     "no_hs"
		local lv2     "has_hs"
		local med_lv1 = 10.0
		local med_lv2 = 19.666667
	}
	if "`layer'" == "gender" {
		local lv1     "female"
		local lv2     "male"
	...
	if "`layer'" == "race" {
		local lv1     "nonwhite"
		local lv2     "white"
		local med_lv1 = 7.333333
		local med_lv2 = 18.0
	}

	* Avg pre-treatment employment per firm × layer (broadcast across all years)
	...
	* Joint indicator
	cap drop in_above_med_both
	gen byte in_above_med_both = (abv_lv1 == 1 & abv_lv2 == 1)
	drop avg_pre_emp abv_lv1 abv_lv2
	label var in_above_med_both ///
		"Both layers above median pre-treatment emp (`lv1'>`med_lv1', `lv2'>`med_lv2')"

	count if in_above_med_both == 1
	di as result "  Above-median both layers: `r(N)' obs"
	...
	* Scale to P90 of untreated balanced-panel above-median firms at 2009.
	local s_spill "lagos_sample_avg==1 & treat_ultra==0 & in_balanced_panel==1 & in_layer_balanced_panel==1 & in_above_med_both==1"
		...
		* ── Event study plot ─────────────────────────────────────────────────
		...
		coefplot _es_tmp, ///
			...
			note("Pre-trend F-test p = `pre_pval_s'  |  Post: `post_coef_s' (`post_se_s')" ///
			     "Sample: above-median employment in both layers") ///
			graphregion(color(white)) bgcolor(white) ///
			ci(95) ciopts(recast(rcap) color(blue)) mcolor(blue)

		graph export "$graphs/layer_connectivity/es_`outcome'_abvmed_spill_`layer'_`d'.pdf", ///
			as(pdf) replace
	...
	****************************************************************************
	...
	foreach outcome in lr_remdezr_layer l_layer_emp {
		...
		* Event study plot
		...
		graph export "$graphs/layer_connectivity/es_`outcome'_abvmed_cross_`layer'_`d'.pdf", ///
			as(pdf) replace
	...
	* Restricted set here = layer-balanced panel AND above-median both layers.
	****************************************************************************
	...
	* ── Identify restricted firm set from current layer data ──────────────
	local s_spill_restr "treat_ultra==0 & in_balanced_panel==1 & in_layer_balanced_panel==1 & in_above_med_both==1 & lagos_sample_avg==1"
	...
	* ── Restrict to layer firms (layer-balanced + above-median) ───────────
	...
	* ── Regressions ──────────────────────────────────────────────────────
	foreach outcome in lr_remdezr_w lr_remdezr_h_w l_firm_emp {
		...
		* Event study plot
		...
		graph export "$graphs/layer_connectivity/es_`outcome'_abvmed_firmrestr_`layer'_`d'.pdf", ///
			as(pdf) replace
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Layer spillover abvmed done" "07c: above-median both-layers restriction complete"

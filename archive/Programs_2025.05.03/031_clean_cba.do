...
gen row_mult = last_active_year-true_start_year+1
expand last_active_year-true_start_year+1 // expands to 1.2M observations
bysort contract_id identificad union_id: replace active_year= true_start_year+(_n-1) // generate active year. The data was previously at the contract_id-union_id-estab level. So these are the right varibles to sort by in order to generate the active year variable.
...
replace treat_ultra=1 if true_start_date<=mdy(9,1,2012) & end_date_stata>=mdy(9,1,2012) // this only marks estabs with active cbas in 2012, does not mark the cross-section of firms that get treated at some point. "If an agreement was reached before Sep 2012"

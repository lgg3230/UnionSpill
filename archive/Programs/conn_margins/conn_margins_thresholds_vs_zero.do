********************************************************************************
* UNION SPILLOVERS — CONNECTIVITY THRESHOLD DUMMIES vs. ZERO CONNECTIVITY
* Purpose: Same as conn_margins_thresholds.do, but each regression compares
*          the above-threshold group against the ZERO-connectivity group only.
*          Firms with connectivity between 0 and the threshold are excluded.
* Output:  Tables/conn_margins/results_thresholds_vs_zero.csv
* Auto-runs: Programs/conn_margins/generate_thresholds_vs_zero_latex.py
********************************************************************************

capture log close
local d = subinstr("`c(current_date)'"," ","_",.)
local t = subinstr("`c(current_time)'",":","",.)
log using "$logs/conn_margins/conn_margins_thresholds_vs_zero_`d'_`t'.log", replace text
...
* Key difference from conn_margins_thresholds.do: each regression restricts
* the sample to (above_`thr' == 1 | totaltreat_pw_n == 0), so the reference
* group is strictly zero-connectivity firms.
********************************************************************************
...
capture erase "$tables/conn_margins/results_thresholds_vs_zero.csv"
tempname fh
file open `fh' using "$tables/conn_margins/results_thresholds_vs_zero.csv", write replace
file write `fh' "threshold,outcome,row_type,value" _n
file close `fh'

foreach thr in p50 p65 p75 p90 {

	* Sample: above threshold OR zero connectivity
	local s_thr "`s_spill' & (above_`thr' == 1 | totaltreat_pw_n == 0)"

	foreach outcome in lr_remdezr_w lr_remdezr_h_w l_firm_emp {

		di as result "Estimating: `outcome' | above_`thr' vs. zero-conn"
		...
		* ── Post-treatment ───────────────────────────────────────────────────
		...
		reghdfe `outcome' i.above_`thr'##i.treat_year if `s_thr', ///
		...
		* ── Pre-treatment placebo ────────────────────────────────────────────
		...
		reghdfe `outcome' i.above_`thr'##i.placebo_year if `s_thr' & year <= 2011, ///
			absorb(`absorb') vce(cluster identificad)
		...
		* ── Pre-trend F-test (event study) ──────────────────────────────────
		...
		reghdfe `outcome' i.above_`thr'##ib2011.year if `s_thr', ///
			absorb(`absorb') vce(cluster identificad)
		...
		* ── Write ────────────────────────────────────────────────────────────
		...
		tempname fh
		file open `fh' using "$tables/conn_margins/results_thresholds_vs_zero.csv", write append
...
shell ~/.conda/envs/venv_python312/bin/python ///
	"$programs/conn_margins/generate_thresholds_vs_zero_latex.py"

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Stata done" "conn_margins_thresholds_vs_zero complete"

...
firm-scale specification with zero-count exclusion (log-compatible).

Outcomes are counts divided by the firm's own pretreatment (2009–2011) mean,
with zero counts excluded from the scaled outcome (i.e., fs_v is only defined
when both the pretreatment mean > 0 and v > 0 in that year).

Output: UnionSpill/Tables/turnover/turnover_firmscale_ll_tables.tex
"""

import re
from pathlib import Path

SPEC = "turnover_firmscale_ll"
...
OUTCOME_GROUPS = [
    dict(
        key="turnover_firmscale_ll",
        direct_caption=(
            r"Direct Effects of the Ultractivity Reform on Employment Flows "
            r"(Firm-Scale Specification, Zero-Excluded)"
        ),
        spill_caption=(
        ...
        direct_notes=notes_direct(
            ...
            r"Unlike the standard firm-scale specification, observations with a "
            r"zero count in a given year are excluded from that outcome's regression, "
            r"making this specification directly comparable to the log-level results. "
        ...
        spill_notes=notes_spill(
            ...
            r"A coefficient of 0.10 indicates the count rose by 10\% relative to "
            r"the establishment's own pretreatment baseline. "
            r"Observations with a zero count in a given year are excluded from "
            r"that outcome's regression. "
...
def main():
    ...
    output_file  = pipeline_dir / "turnover_firmscale_ll_tables.tex"
    ...
    for g in OUTCOME_GROUPS:
        ...
        g["panel_c"]       = panel_c
        g["spill"]         = spill
        g["spill_outcomes"]    = SPILL_OUTCOMES
        g["spill_col_headers"] = SPILL_HEADERS

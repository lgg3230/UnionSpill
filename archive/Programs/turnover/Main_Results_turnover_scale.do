...
*          results_direct_panelA_turnover_scale.csv
*          results_direct_panelB_turnover_scale.csv
*          results_direct_panelC_turnover_scale.csv
*          results_spill_turnover_scale.csv
...
log using "$logs/turnover/FinalResults_turnover_scale_`d'_`t'.log", replace text
...
* ── c) PRE-TREATMENT MEANS + 4-BIN CONTROLS FOR TURNOVER OUTCOMES ─────────────
...
foreach panel in A B C {
	capture erase "$tables/turnover/results_direct_panel`panel'_turnover_scale.csv"
	tempname fh
	file open `fh' using "$tables/turnover/results_direct_panel`panel'_turnover_scale.csv", write replace
	file write `fh' "spec,section,outcome,row_type,value" _n
	file close `fh'
}

capture erase "$tables/turnover/results_spill_turnover_scale.csv"
tempname fh
file open `fh' using "$tables/turnover/results_spill_turnover_scale.csv", write replace
file write `fh' "spec,section,outcome,row_type,value" _n
file close `fh'
...
foreach panel in A B C {
	...
	local csv_out "$tables/turnover/results_direct_panel`panel'_turnover_scale.csv"
	...
	* panel sample (treated + control as defined for this panel), 2009-2011.
	...
	foreach outcome of global turnover_outcomes {
...
local csv_spill "$tables/turnover/results_spill_turnover_scale.csv"
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Stata done" "turnover_scale regressions complete"

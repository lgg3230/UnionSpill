"""
treated_cba_self_similarity_prep.py

MIRROR of cba_self_similarity_prep.py for the treated side.

Computes within-firm CBA self-similarity for treated firms: for each firm i
and CBA period t, similarity between firm i's clause vector at period t and
its OWN clause vector at the last pre-Sumula period (cba_period == 2).

Reads from cba_clauses_by_period_treated.dta (exported by
treated_cba_self_similarity.do), which keeps the treated balanced-panel
firms; their period-2 vector serves as their own pre-treatment reference.

Inputs:
  Data/RAIS_aux/cba_clauses_by_period_treated.dta

Output:
  Data/RAIS_aux/treated_cba_self_similarity_panel.dta
    one row per (treated firm, cba_period) with non-missing self-reference;
...
sample_path  = rais_aux / "treated_cba_similarity_panel.dta"
output_path  = rais_aux / "treated_cba_self_similarity_panel.dta"
...
# ── Load (identificad, cba_period) keys from treated-spillover sample ─────────
# Match the (firm, period) sample used in treated_cba_similarity.do. We
# inner-merge on (identificad, cba_period) at the END so that every row in
# this output panel also exists in treated_cba_similarity_panel.dta — the
# regression sample is identical to treated_cba_similarity_table.tex.
print("Loading sample keys from treated_cba_similarity_panel.dta...", flush=True)
...
print(f"  {len(sample_keys):,} firm-period rows in spillover sample "
      f"({sample_keys['identificad'].nunique():,} treated firms)", flush=True)

# ── Build per-firm period-2 reference vector (treated firms only) ─────────────
print("Building period-2 self-reference vectors (treated firms)...", flush=True)
ref = clauses[(clauses["cba_period"] == 2) & (clauses["treat_ultra"] == 1)][
...
print(f"  {len(ref):,} treated firms with a period-2 CBA", flush=True)

# Restrict to treated firms only and merge their own period-2 vector
...
print(f"  Treated firm-period obs with a period-2 self-reference: "
...
# Inner-merge on (identificad, cba_period) with the treated moving-reference
# panel so the regression sample is identical to treated_cba_similarity_table.tex.
n_before = len(result)
...
print(f"  Firm-period obs: {n_before:,} → {len(result):,} after inner-merge "
      f"(dropped {n_before - len(result):,} rows not in treated moving-ref sample)",
      flush=True)
...
subprocess.run(
    ["curl", "-s", "-o", "/dev/null",
     "-H", "Title: treated_cba_self_similarity_prep done",
     "-d", f"Saved {len(result):,} treated obs to "
           f"treated_cba_self_similarity_panel.dta",

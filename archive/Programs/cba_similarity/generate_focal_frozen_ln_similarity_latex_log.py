"""
generate_focal_frozen_ln_similarity_latex.py

Log-similarity table for Exercise B (focal frozen at p2, partner moving).
"""
...
csv_path   = tables_dir / "results_spill_focal_frozen_ln_cba_similarity.csv"
tex_path   = tables_dir / "focal_frozen_ln_cba_similarity_table.tex"
...
base_spec  = "focal_frozen_ln_cba_similarity_tfpw_07_11"
union_spec = "focal_frozen_ln_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{Log CBA Content Similarity: Focal Firm Frozen at Period 2, Treated Partners Moving}",
    r"\label{tab:focal_frozen_ln_cba_similarity}",
    r"\scriptsize",
    ...
    r"\smallskip",
    (r"\scriptsize\textit{Notes:} This table reports Exercise B with log "
     r"similarity outcomes. The focal (spillover) firm's clause vector is "
     r"\emph{frozen} at \textbf{cba\_period $= 2$}; the treated-partner "
     r"reference moves with the current period. Outcomes are $\ln(\text{sim}_{it})$ "
     r"for $\text{sim} \in \{\text{cosine}, \text{Bray-Curtis}, "
     r"\text{total variation}, \text{Ruzicka}\}$, dropping firm-period "
     r"observations with zero similarity. The coefficient is interpretable "
     r"as the percentage change in similarity associated with a one-unit "
     r"increase in connectivity. Sample, weights, and fixed-effects structure "
     r"are identical to Table~\ref{tab:focal_frozen_cba_similarity}; see notes "
     r"there. Standard errors clustered at the establishment level. "

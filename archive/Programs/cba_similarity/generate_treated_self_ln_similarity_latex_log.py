"""
generate_treated_self_ln_similarity_latex.py
MIRROR of generate_treated_ln_similarity_latex.py.

Reads results_spill_treated_self_ln_cba_similarity.csv and produces a LaTeX
table for the treated-side self-similarity exercise (treated firms vs. their
OWN period-2 CBA — within-firm content stability, log).
"""
...
csv_path   = tables_dir / "results_spill_treated_self_ln_cba_similarity.csv"
tex_path   = tables_dir / "treated_self_ln_cba_similarity_table.tex"
...
base_spec  = "treated_self_ln_cba_similarity_tfpw_07_11"
union_spec = "treated_self_ln_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{Log CBA Content Stability: Treated Firms vs.\ Own Pre-Treatment CBA}",
    r"\label{tab:treated_self_ln_cba_similarity}",
    r"\scriptsize",
    ...
    (r"\textit{Notes:} This table reports estimates of \emph{log} within-firm "
     r"CBA content stability on connectivity for treated firms. Outcomes are "
     r"$\ln(\text{sim}_{it})$ for $\text{sim} \in \{\text{cosine}, "
     r"\text{Bray-Curtis}, \text{total variation}, \text{Ruzicka}\}$, dropping "
     r"firm-period observations with zero similarity. Each similarity is "
     r"computed between firm $i$'s clause vector at period $t$ and firm $i$'s "
     r"OWN clause vector at cba\_period $= 2$. Sample, connectivity, and "
     r"fixed-effects structure are identical to "
     r"Table~\ref{tab:treated_self_cba_similarity}; see notes there. "

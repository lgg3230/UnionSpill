"""
generate_pretreat_ref_uncorr_w_ln_similarity_latex.py
MIRROR of generate_ln_similarity_latex.py.

Reads results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv and produces a LaTeX
table with one column per log-similarity measure (untreated firms vs. fixed
...
csv_path   = tables_dir / "results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv"
tex_path   = tables_dir / "pretreat_ref_uncorr_w_ln_cba_similarity_table.tex"
...
base_spec  = "pretreat_ref_uncorr_w_ln_cba_similarity_tfpw_07_11"
union_spec = "pretreat_ref_uncorr_w_ln_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{Log CBA Content Similarity to Fixed Pre-Treatment Treated Reference}",
    r"\label{tab:pretreat_ref_uncorr_w_ln_cba_similarity}",
    r"\scriptsize",
    ...
    (r"\scriptsize\textit{Notes:} This table reports spillover-effect estimates "
     ...
     r"vector is fixed at the treated partners' \textit{pre-Sumula 277} contracts "
     ...
     r"associated with a one-unit increase in connectivity. Sample, weights, "
     r"and fixed-effects structure are identical to "
     r"Table~\ref{tab:pretreat_ref_uncorr_w_cba_similarity}; see notes there. "

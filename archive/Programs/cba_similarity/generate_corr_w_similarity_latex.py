"""
generate_corr_w_similarity_latex.py
MIRROR of generate_similarity_latex.py.

Reads results_spill_corr_w_cba_similarity.csv and produces a LaTeX table
with one column per similarity measure (untreated firms vs. moving
...
csv_path   = tables_dir / "results_spill_corr_w_cba_similarity.csv"
tex_path   = tables_dir / "corr_w_cba_similarity_table.tex"
...
base_spec  = "corr_w_cba_similarity_tfpw_07_11"
union_spec = "corr_w_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{CBA Similarity to Moving Treated Reference (Orientation-Corrected Weights)}",
    r"\label{tab:corr_w_cba_similarity}",
    r"\scriptsize",
    ...
    (r"\scriptsize\textit{Notes:} This table reproduces the spillover "
     r"similarity exercise of Table~\ref{tab:cba_similarity} (CBA content "
     r"similarity vs.\ flow-weighted treated reference) with "
     r"\textbf{orientation-corrected connectivity weights}. For each "
     r"untreated firm $i$ and CBA period $t$, the outcome is the similarity "
     r"between firm $i$'s clause vector at period $t$ and the flow-weighted "
     r"average of treated partners' clause vectors at the SAME period $t$. "
     r"Weights are now $w_{ij} = \frac{1}{|T|} \sum_t \frac{\text{flows}_t(i,j)}"
     r"{\overline{\text{emp}}_{i,t}}$, computed from firm $i$'s OWN average "
     r"employment each year-pair (correcting the i$<$j orientation bias of "
     r"\texttt{bilateral\_conn\_pw}). Sample, fixed-effects, and standard-error "
     r"clustering are identical to Table~\ref{tab:cba_similarity}; see notes "
     r"there. *\,$p<0.10$, **\,$p<0.05$, ***\,$p<0.01$."),
    r"\end{minipage}",
    r"\end{table}",
]

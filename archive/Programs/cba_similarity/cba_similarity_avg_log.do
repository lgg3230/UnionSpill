...
* where j ranges over treated firms with bilateral flows to i (w_{ij}).
*
* Similarity measures: cosine, bray_curtis, total_variation, ruzicka
...
*   3. Python computes reference vectors + similarities (cba_similarity_avg_prep.py)
*   4. Load Python output, merge, run panel regressions
*
* Output:
*   Tables/cba_similarity/results_direct_panel[A/B/C]_cba_similarity_tfpw_07_11.csv
*   Tables/cba_similarity/results_spill_cba_similarity_avg_tfpw_07_11.csv
*   Tables/cba_similarity/cba_similarity_avg_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/cba_similarity_avg_`d'_`t'.log", replace text
...
* Computes flow-weighted reference vectors and 4 similarity measures.
* Output: Data/RAIS_aux/cba_similarity_avg_panel.dta
********************************************************************************

di as result "Running Python similarity prep..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/cba_similarity_avg_prep.py"
...
di as result "Merging similarity measures into panel..."

* Merge similarity measures (one obs per untreated firm × cba_period)
merge m:1 identificad cba_period using "$rais_aux/cba_similarity_avg_panel.dta", ///
	keep(master match) gen(m_sim)
drop m_sim

label var cosine          "Cosine similarity to connected treated avg"
label var bray_curtis     "Bray-Curtis similarity to connected treated avg"
label var total_variation "Total variation similarity to connected treated avg"
label var ruzicka         "Ruzicka similarity to connected treated avg"

local sim_outcomes "cosine bray_curtis total_variation ruzicka"

di as result "Similarity merge complete. Observations with similarity:"
...
capture erase "$tables/cba_similarity_log/results_spill_cba_similarity_avg_tfpw_07_11.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_cba_similarity_avg_tfpw_07_11.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_ln_cba_similarity_avg_tfpw_07_11.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_ln_cba_similarity_avg_tfpw_07_11.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'
...
local csv_spill "$tables/cba_similarity_log/results_spill_cba_similarity_avg_tfpw_07_11.csv"
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — LOG SIMILARITY"
di as result "-----------------------------------------------------------------------"

local spec_ln    "ln_cba_similarity_avg_tfpw_07_11"
local csv_ln     "$tables/cba_similarity_log/results_spill_ln_cba_similarity_avg_tfpw_07_11.csv"
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_similarity_avg_latex.py"
di as result "LaTeX table written to Tables/cba_similarity/cba_similarity_avg_table.tex"

shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_ln_similarity_avg_latex_log.py"
di as result "LaTeX table written to Tables/cba_similarity/ln_cba_similarity_avg_table.tex"

shell source "$programs/notify.sh" && notify "cba_similarity_avg done (log1p)" "CBA similarity (avg treated ref) regressions and table complete"

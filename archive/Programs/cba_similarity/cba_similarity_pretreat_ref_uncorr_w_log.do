********************************************************************************
* cba_similarity_pretreat_ref_uncorr_w.do
*
* MIRROR of cba_similarity.do, with the similarity outcome computed against a
* FIXED pre-treatment reference vector (treated partners' clauses in
* cba_period == 2) instead of a moving same-period reference.
*
* For each (untreated firm i, cba_period t):
*   sim_{it} = sim(x_{it}, ref_i)
* where ref_i = Sigma_{j in T(i)} w_{ij} * x_{j,2} / Sigma_{j in T(i)} w_{ij}
* j ranges over treated firms with bilateral flows to i (w_{ij}, focal-firm
...
*      (cba_similarity_pretreat_ref_uncorr_w_prep.py)
*   4. Merge similarity panel and run panel regressions
*
* Output:
*   Tables/cba_similarity/results_spill_pretreat_ref_uncorr_w_cba_similarity.csv
*   Tables/cba_similarity/results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv
*   Tables/cba_similarity/pretreat_ref_uncorr_w_cba_similarity_table.tex
*   Tables/cba_similarity/pretreat_ref_uncorr_w_ln_cba_similarity_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/cba_similarity_pretreat_ref_uncorr_w_`d'_`t'.log", replace text
...
* four similarity measures. Output: cba_similarity_pretreat_ref_uncorr_w_panel.dta.
********************************************************************************

di as result "Running Python pretreat-ref similarity prep..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/cba_similarity_pretreat_ref_uncorr_w_prep.py"
...
di as result "Merging pretreat-ref similarity measures into panel..."

merge m:1 identificad cba_period using "$rais_aux/cba_similarity_pretreat_ref_uncorr_w_panel.dta", ///
	keep(master match) gen(m_sim)
drop m_sim

label var cosine          "Cosine similarity to fixed pre-treat treated avg"
label var bray_curtis     "Bray-Curtis similarity to fixed pre-treat treated avg"
label var total_variation "Total variation similarity to fixed pre-treat treated avg"
label var ruzicka         "Ruzicka similarity to fixed pre-treat treated avg"

local sim_outcomes "cosine bray_curtis total_variation ruzicka"

di as result "Pretreat-ref similarity merge complete. Observations with similarity:"
...
local spec             "pretreat_ref_uncorr_w_cba_similarity_tfpw_07_11"
...
capture erase "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — PRETREAT REFERENCE"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local sim_outcomes {
		...
		di as text "  Estimating: `outcome' (spillover, pretreat-ref, `fe_variant')"
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — LOG SIMILARITY (PRETREAT REFERENCE)"
di as result "-----------------------------------------------------------------------"

local spec_ln    "pretreat_ref_uncorr_w_ln_cba_similarity_tfpw_07_11"
local csv_ln     "$tables/cba_similarity_log/results_spill_pretreat_ref_uncorr_w_ln_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local ln_sim_outcomes {
		...
		di as text "  Estimating: `outcome' (spillover, log, pretreat-ref, `fe_variant')"
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_pretreat_ref_uncorr_w_similarity_latex.py"
di as result "LaTeX table written to Tables/cba_similarity/pretreat_ref_uncorr_w_cba_similarity_table.tex"

shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_pretreat_ref_uncorr_w_ln_similarity_latex_log.py"
di as result "LaTeX table written to Tables/cba_similarity/pretreat_ref_uncorr_w_ln_cba_similarity_table.tex"

shell source "$programs/notify.sh" && notify "cba_similarity_pretreat_ref_uncorr_w done (log1p)" "Pretreat-ref similarity regressions and tables complete"

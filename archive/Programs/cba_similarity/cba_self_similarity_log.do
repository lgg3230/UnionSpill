********************************************************************************
* cba_self_similarity.do
*
* MIRROR of cba_similarity.do, with the similarity outcome computed against the
* firm's OWN period-2 CBA (within-firm content stability).
*
* For each (untreated firm i, cba_period t):
*   sim_{it} = sim(x_{it}, x_{i,2})
* where x_{i,2} is firm i's clause vector at cba_period == 2 (the last
* pre-Sumula CBA). The hypothesis: untreated firms drift little from their
* own past, so their connectivity-by-post interaction should be ~zero.
*
* Pipeline:
*   1. Load + setup variables (mirrors cba_similarity.do)
*   2. Export intermediate clause data (cba_clauses_by_period.dta)
*   3. Python computes self-similarities (cba_self_similarity_prep.py)
*   4. Merge similarity panel and run panel regressions
*
* Output:
*   Tables/cba_similarity/results_spill_self_cba_similarity.csv
*   Tables/cba_similarity/results_spill_self_ln_cba_similarity.csv
*   Tables/cba_similarity/self_cba_similarity_table.tex
*   Tables/cba_similarity/self_ln_cba_similarity_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/cba_self_similarity_`d'_`t'.log", replace text
...
* Computes within-firm self-similarity. Output: cba_self_similarity_panel.dta.
********************************************************************************

di as result "Running Python self-similarity prep..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/cba_self_similarity_prep.py"
...
di as result "Merging self-similarity measures into panel..."

merge m:1 identificad cba_period using "$rais_aux/cba_self_similarity_panel.dta", ///
	keep(master match) gen(m_sim)
drop m_sim

label var cosine          "Cosine similarity to own period-2 CBA"
label var bray_curtis     "Bray-Curtis similarity to own period-2 CBA"
label var total_variation "Total variation similarity to own period-2 CBA"
label var ruzicka         "Ruzicka similarity to own period-2 CBA"

local sim_outcomes "cosine bray_curtis total_variation ruzicka"

di as result "Self-similarity merge complete. Observations with similarity:"
...
local spec             "self_cba_similarity_tfpw_07_11"
...
capture erase "$tables/cba_similarity_log/results_spill_self_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_self_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_self_ln_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_self_ln_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

********************************************************************************
* SECTION 9: SPILLOVER EFFECTS — SELF-SIMILARITY (LEVELS)
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — SELF-SIMILARITY (UNTREATED)"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/cba_similarity_log/results_spill_self_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local sim_outcomes {
		...
		di as text "  Estimating: `outcome' (self, untreated, `fe_variant')"
...
********************************************************************************
* SECTION 9b: SPILLOVER EFFECTS — SELF-SIMILARITY (LOG)
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — LOG SELF-SIMILARITY (UNTREATED)"
di as result "-----------------------------------------------------------------------"

local spec_ln    "self_ln_cba_similarity_tfpw_07_11"
local csv_ln     "$tables/cba_similarity_log/results_spill_self_ln_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local ln_sim_outcomes {
		...
		di as text "  Estimating: `outcome' (self, log, untreated, `fe_variant')"
...
di as result _newline "Log self-similarity regressions complete."
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_self_similarity_latex.py"
di as result "LaTeX table written to Tables/cba_similarity/self_cba_similarity_table.tex"

shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_self_ln_similarity_latex_log.py"
di as result "LaTeX table written to Tables/cba_similarity/self_ln_cba_similarity_table.tex"

shell source "$programs/notify.sh" && notify "cba_self_similarity done (log1p)" "Self-similarity (untreated) regressions and tables complete"

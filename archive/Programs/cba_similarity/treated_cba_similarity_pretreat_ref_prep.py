...
MIRROR of cba_similarity_pretreat_ref_prep.py with focal/partner roles swapped.
...
weighted by orientation-corrected per-worker connectivity. Similarity between
the focal firm's actual clauses (in each cba_period) and that fixed reference
answers: are treated firms moving toward what their connected untreated
partners' contracts looked like before treatment?

Inputs:
  Data/RAIS_aux/cba_clauses_by_period_treated.dta    (exported by treated_cba_similarity_pretreat_ref.do)
...
Output:
  Data/RAIS_aux/treated_cba_similarity_pretreat_ref_panel.dta
...
output_path    = rais_aux  / "treated_cba_similarity_pretreat_ref_panel.dta"
...
# Filter: focal treated, partner untreated  (RECIPROCAL direction)
pairs = pairs[(pairs["focal"].map(treat_status) == 1) &
              (pairs["partner"].map(treat_status) == 0)].copy()
print(f"  Treated→untreated pairs (orientation-corrected): {len(pairs):,}")
...
subprocess.run(
    ["curl", "-s", "-o", "/dev/null",
     "-H", "Title: treated_cba_similarity_pretreat_ref_prep done",
     "-d", f"Saved {len(result):,} treated obs to "
           f"treated_cba_similarity_pretreat_ref_panel.dta",

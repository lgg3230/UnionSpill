...
* ---- DETERMINISM: canonical row order, stage 1 (one spell per worker-firm) ----
* Same defect as 1010/1060: the seeded runiform() below assigns draws in physical
* row order, so ties resolve differently whenever the input order changes. Sort on
* a key whose prefix is `identificad PIS` (so the bysort operations that follow are
* order-preserving no-ops) and which contains every variable carried forward out of
* this stage. Rows still tied are identical on all of them.
* See Docs/pipeline/REDUNDANCY_AUDIT.md D2.
sort identificad PIS horascontr remdezr tempempr
...
* MATLAB stage. Paths are parameterised rather than hardcoded so the chain can be
* exercised against a scratch copy without writing into production Data/RAIS_aux.
* UNIONSPILL_AUX is read by 1051-1055, which fall back to the production path when
* it is unset, so behaviour is unchanged for a normal run.
if "$matlab_exe" == "" global matlab_exe "/software/matlab/R2020b/bin/matlab"
if "$programs"   == "" global programs   "/kellogg/proj/lgg3230/UnionSpill/Programs"

foreach mscript in 1051_connectivity_full_lagos 1052_connectivity_treat_lagos ///
                   1053_connectivity_control_lagos 1054_connectivity_treat_onecba ///
                   1055_connectivity_treat_zerocba {
    di as result "[1050] MATLAB: `mscript'"

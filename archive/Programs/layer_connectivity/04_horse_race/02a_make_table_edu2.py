...
def build_latex(data: dict, caption: str, label: str) -> str:
    ...
    def row(label, rt_no_hs, rt_has_hs, fmt_fn=fmt_coef, end=r" \\"):
        cells = [label]
        for _, section, outcome in ALL_COLS:
            cells.append(fmt_fn(get(data, section, outcome, rt_no_hs)))
        # second row type (has_hs) — only used when we have two different rt args
        # Actually for SE rows we call row() with the SE row_type directly

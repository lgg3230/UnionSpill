"""
Assemble horse race tables (cross-firm FE, above-p25 both layers)
from 13f_horse_race_xfirm_abvp25.do output — one table per layer.

Reads:
  Tables/layer_connectivity/results_horse_race_{edu2,gender,race}_xfirm_abvp25.csv

Outputs (per layer):
  Tables/layer_connectivity/table_horse_race_{edu2,gender,race}_xfirm_abvp25.tex / .csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/14h_make_table_horse_race_xfirm_abvp25.py
"""
...
LAYER_CONFIG = {
    "edu2": {
        ...
        "caption":    r"Horse race: $c_{\text{no\_hs}}$ vs.\ $c_{\text{has\_hs}}$ simultaneously (cross-firm FE, above-p25 both layers)",
    ...
    "gender": {
        ...
        "lv2_tex":    "male",
        "panel_desc": "gender",
        "caption":    r"Horse race: $c_{\text{female}}$ vs.\ $c_{\text{male}}$ simultaneously (cross-firm FE, above-p25 both layers)",
    ...
    "race": {
        ...
        "lv2_tex":    "white",
        "panel_desc": "race",
        "caption":    r"Horse race: $c_{\text{nonwhite}}$ vs.\ $c_{\text{white}}$ simultaneously (cross-firm FE, above-p25 both layers)",
...
for layer in LAYERS:
    ...
    if not csv_in.exists():
        print(f"WARNING: {csv_in.name} not found — skipping {layer}. Run 13f first.")
        continue

...
*
* Output:  Tables/layer_connectivity/results_spill_layer_{edu2,gender,race}_lbal_layer_spill.csv
...
log using "$logs/layer_connectivity/layer_spillover_lbal_`d'_`t'.log", replace text

di "Started: `c(current_date)' `c(current_time)'"
di "Stata version: `c(stata_version)'"

global layer_data "$main/UnionSpill/Data/layer_connectivity"
local  spec       "lbal_layer_spill"
...
foreach layer in edu2 gender race {
	...
	* Data is sparse: a row exists only when layer employment is positive.
	* A firm is in the layer-balanced panel if every layer is observed in all
	* 8 years from 2009 to 2016.
	...
	* Keep unique firms satisfying s_spill with non-missing layer connectivity.
	local s_spill "treat_ultra==0 & in_balanced_panel==1 & in_layer_balanced_panel==1 & lagos_sample_avg==1"
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Layer spillover done" "07b: edu2 + gender + race regressions complete"

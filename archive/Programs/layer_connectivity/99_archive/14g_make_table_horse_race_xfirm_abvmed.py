"""
Assemble horse race tables (cross-firm FE, above-median both layers)
from 13e_horse_race_xfirm_abvmed.do output — one table per layer.

Reads:
  Tables/layer_connectivity/results_horse_race_{edu2,gender,race}_xfirm_abvmed.csv

Outputs (per layer):
  Tables/layer_connectivity/table_horse_race_{edu2,gender,race}_xfirm_abvmed.tex / .csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/14g_make_table_horse_race_xfirm_abvmed.py
"""
...
LAYER_CONFIG = {
    "edu2": {
        ...
        "caption":    r"Horse race: $c_{\text{no\_hs}}$ vs.\ $c_{\text{has\_hs}}$ simultaneously (cross-firm FE, above-median both layers)",
    ...
    "gender": {
        ...
        "lv2_tex":    "male",
        "panel_desc": "gender",
        "caption":    r"Horse race: $c_{\text{female}}$ vs.\ $c_{\text{male}}$ simultaneously (cross-firm FE, above-median both layers)",
    ...
    "race": {
        ...
        "lv2_tex":    "white",
        "panel_desc": "race",
        "caption":    r"Horse race: $c_{\text{nonwhite}}$ vs.\ $c_{\text{white}}$ simultaneously (cross-firm FE, above-median both layers)",
...
for layer in LAYERS:
    ...
    csv_in  = TABLES / f"results_horse_race_{layer}_xfirm_abvmed.csv"
    tex_out = TEX_TABLES / f"table_horse_race_{layer}_xfirm_abvmed.tex"
    csv_out = TABLES / f"table_horse_race_{layer}_xfirm_abvmed.csv"

    if not csv_in.exists():
        print(f"WARNING: {csv_in.name} not found — skipping {layer}. Run 13e first.")
        continue

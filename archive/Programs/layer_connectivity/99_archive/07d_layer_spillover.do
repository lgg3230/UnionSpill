********************************************************************************
* UNION SPILLOVERS — LAYER-LEVEL SPILLOVER EFFECTS (ABOVE-P25 BOTH LAYERS)
* Purpose: Same as 07c_layer_spillover.do but restricted to firms where BOTH
*          layers have average pre-treatment employment (2009-2011) above the
*          25th percentile for that split.
*
*   Hardcoded p25s (from firm_layer_outcomes data, 2009-2011):
...
*
* Output:  Tables/layer_connectivity/results_spill_layer_{edu2,gender,race}_abvp25_layer_spill.csv
*          Tables/layer_connectivity/results_spill_layer_cross_{edu2,gender,race}_abvp25_layer_spill.csv
*          Tables/layer_connectivity/results_spill_firmrestr_{edu2,gender,race}_abvp25_layer_spill.csv
*          Graphs/layer_connectivity/es_{outcome}_abvp25_{spill,cross,firmrestr}_{layer}_{date}.pdf
********************************************************************************
...
log using "$logs/layer_connectivity/layer_spillover_abvp25_`d'_`t'.log", replace text

di "Started: `c(current_date)' `c(current_time)'"
di "Stata version: `c(stata_version)'"

global layer_data "$main/UnionSpill/Data/layer_connectivity"
local  spec       "abvp25_layer_spill"
...
foreach layer in edu2 gender race {
	...
	* P25s of avg pre-treatment (2009-2011) employment, computed across all
	...
	* Scale to P90 of untreated balanced-panel above-p25 firms at 2009.
	local s_spill "lagos_sample_avg==1 & treat_ultra==0 & in_balanced_panel==1 & in_layer_balanced_panel==1 & in_above_p25_both==1"
		...
		* ── Event study plot ─────────────────────────────────────────────────
		...
		coefplot _es_tmp, ///
			...
			note("Pre-trend F-test p = `pre_pval_s'  |  Post: `post_coef_s' (`post_se_s')" ///
			     "Sample: above-p25 employment in both layers") ///
			graphregion(color(white)) bgcolor(white) ///
			ci(95) ciopts(recast(rcap) color(blue)) mcolor(blue)

		graph export "$graphs/layer_connectivity/es_`outcome'_abvp25_spill_`layer'_`d'.pdf", ///
			as(pdf) replace
	...
	****************************************************************************
	...
	foreach outcome in lr_remdezr_layer l_layer_emp {
		...
		* Event study plot
		...
		graph export "$graphs/layer_connectivity/es_`outcome'_abvp25_cross_`layer'_`d'.pdf", ///
			as(pdf) replace
	...
	* Restricted set here = layer-balanced panel AND above-p25 both layers.
	****************************************************************************
	...
	* ── Identify restricted firm set from current layer data ──────────────
	local s_spill_restr "treat_ultra==0 & in_balanced_panel==1 & in_layer_balanced_panel==1 & in_above_p25_both==1 & lagos_sample_avg==1"
	...
	* ── Restrict to layer firms (layer-balanced + above-p25) ───────────
	...
	* ── Regressions ──────────────────────────────────────────────────────
	foreach outcome in lr_remdezr_w lr_remdezr_h_w l_firm_emp {
		...
		* Event study plot
		...
		graph export "$graphs/layer_connectivity/es_`outcome'_abvp25_firmrestr_`layer'_`d'.pdf", ///
			as(pdf) replace
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Layer spillover abvp25 done" "07d: above-p25 both-layers restriction complete"

"""
Assemble horse race table (cross-firm FE) from 13b_horse_race_edu2_xfirm.do output.

Identical layout to 14_make_table_horse_race.py but reads cross-firm sections:
  layer_cross_firm  (instead of layer_firm_year)
  firm_cross_firm   (instead of firm_firm_year)

Output:
  Tables/layer_connectivity/table_horse_race_edu2_xfirm.tex / .csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/14b_make_table_horse_race_xfirm.py
"""
...
# ── Column definitions (cross-firm section keys) ──────────────────────────────
WAGE_COLS = [
    ("No HS",  "layer_cross_firm", "lr_remdezr_layer_no_hs"),
    ("Has HS", "layer_cross_firm", "lr_remdezr_layer_has_hs"),
    ("Firm",   "firm_cross_firm",  "lr_remdezr_w"),
]
EMP_COLS = [
    ("No HS",  "layer_cross_firm", "l_layer_emp_no_hs"),
    ("Has HS", "layer_cross_firm", "l_layer_emp_has_hs"),
...
def build_latex(data: dict, caption: str, label: str) -> str:
    ...
    for stat_label, rt, fmt_fn in [
        ...
        ("Firms",                      "n_firms",            fmt_n),
        (r"Pre-F $p$: $c_{\text{no\_hs}}$",  "c_no_hs_pre_ftest",  lambda v: fmt_stat(v, is_pval=True)),
        (r"Pre-F $p$: $c_{\text{has\_hs}}$", "c_has_hs_pre_ftest", lambda v: fmt_stat(v, is_pval=True)),
    ]:
        cells = [stat_label]
        for _, section, outcome in ALL_COLS:
...
if not CSV_IN.exists():
    raise SystemExit(f"ERROR: results file not found — run 13b_horse_race_edu2_xfirm.do first:\n  {CSV_IN}")

#!/usr/bin/env python3
"""
Generate combined LaTeX table for demographic robustness (outcome: lr_remdezr_w).

Columns:
  (1) Baseline                  — demo_controls col 1 (or demo_linear col 1, identical)
  (2) Quartile bins × Year      — demo_controls col 6 (all demographics, absorbed bins)
  (3) Linear × Year             — demo_linear col 2 (all demographics, linear covariates)

Output: Tables/robustness/demo_combined_table.tex
"""
...
def make_table(bins, linear):
    # Map to display columns: (data_source, csv_col)
    cols = [
        ("bins",   "1"),   # (1) baseline
        ("bins",   "6"),   # (2) all quartile bins
        ("linear", "2"),   # (3) all linear
    ]
    ...
    notes = (
        ...
        r"specification. Column~(2) adds quartile-bin controls (absorbed as "
        r"fixed effects, interacted with year) for gender (\% male), race "
        r"(\% white), education (\% with at least high school), average age, "
        r"and average tenure. Column~(3) adds the same five characteristics "
        r"as continuous linear controls interacted with year dummies. "
    ...
    lines = [
        ...
        r"\label{tab:spill_demo_combined}",
        r"\scriptsize",
...
def main():
    ...
    bins   = load_csv(rob_dir / "results_spill_demo_controls.csv")
    linear = load_csv(rob_dir / "results_spill_demo_linear.csv")

    table = make_table(bins, linear)

    output_file = rob_dir / "demo_combined_table.tex"

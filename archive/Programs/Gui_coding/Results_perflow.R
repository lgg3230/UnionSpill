...
#### USING THE PER FLOW MEASURE OF CONNECTIVITY 
...
rm(network_size,totalflows_pw_n_data)
gc()
...
## Imputing zero for per-flow connectivity measure of observations with NA connectivity
df <- df %>% mutate(totaltreat_pf_n = replace_na(totaltreat_pf_n, 0))
...
p90_conn <- df %>%
  ...
  summarise(p90 = quantile(totaltreat_pf_n, 0.90, na.rm = TRUE)) %>%
  pull(p90)

df <- df %>% mutate(totaltreat_pf_norm = totaltreat_pf_n / p90_conn)
...
direct_a <- df %>%
  filter(in_balanced_panel == 1,
         treat_ultra == 1 | (treat_ultra == 0 & totaltreat_pf_n == 0))

# # Direct effects Panel A one <1% connectivity
# # (excludes indirectly exposed firms from the control group)
# direct_a <- df %>%
#   filter(in_balanced_panel == 1,
#          treat_ultra == 1 | (treat_ultra == 0 & totaltreat_pf_n <0.01))
...
es_spill <- feols(
  as.formula(paste0(
    "lr_remdezr_w ~ i(year, totaltreat_pf_norm, ref = 2011) | ",
    fe_specs[["lr_remdezr_w"]])),

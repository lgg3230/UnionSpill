...
## read data for 6-yr connectivity
six_yr <- read_dta(file.path(rais_firm,
                             "connectivity_treat_2005_2011_6yr_sample.dta"))


# Merge network size to main data
...
df <- df %>% left_join(six_yr,
                       by = c("identificad"))

rm(network_size,totalflows_pw_n_data,six_yr)
gc()
...
# NOTE: Using 6-year connectivity measure (totaltreat_pw_n_6yr)
p90_conn <- df %>%
  filter(treat_ultra == 0, in_balanced_panel == 1, year == 2009) %>%
  summarise(p90 = quantile(totaltreat_pw_n_6yr, 0.90, na.rm = TRUE)) %>%
  pull(p90)

df <- df %>% mutate(totaltreat_pw_norm = totaltreat_pw_n_6yr / p90_conn)
...
direct_a <- df %>%
  filter(in_balanced_panel == 1,
         treat_ultra == 1 | (treat_ultra == 0 & totaltreat_pw_n_6yr == 0))
...
## Best-looking out of reasonable ones (in the 4yr version)
# fe_specs <- list(
#   lr_remdezr_w   = paste(base_controls_year,
...
fe_specs <- list(
  lr_remdezr_w   = paste(base_controls_year,
                         "+ lr_remdezr_w_pre4^year+ l_firm_emp_pre4^turnover_pre4^year"),
  lr_remdezr_h_w = paste(base_controls_year,
                         "+ lr_remdezr_h_w_pre4^year + l_firm_emp_pre4^turnover_pre4^year"),
  l_firm_emp     = paste(base_controls_year,
                         "+ l_firm_emp_pre4^year + l_firm_emp_pre4^turnover_pre4^year"),
  numb_clauses   = paste(base_controls_cba,
                         "+ numb_clauses_pre4^cba_period + l_firm_emp_pre4^turnover_pre4^cba_period")
)
...
fig1 <- ggplot(es_df, aes(x = year, y = Estimate)) +
  ...
  labs(x = NULL, y = "Dynamic DiD coefficients",
       title = "Figure 1: Direct Effects on Log Wages (6yr connectivity)",
...
fig2 <- ggplot(es_sp_df, aes(x = year, y = Estimate)) +
  ...
  annotate("text", x = 2015, y = max(es_sp_df$ci_hi) * 0.85,
           ...
           color = "blue", size = 3.5) +
  labs(x = NULL, y = "Dynamic DiD coefficients",
       title = "Figure 2: Spillover Effects on Log Wages (6yr connectivity)",
...
print_table(tab2, "Table 2: Direct Effects (6yr connectivity)")
print_table(tab3, "Table 3: Spillover Effects (6yr connectivity)")
print_table(tab4, "Table 4: Spillover Robustness to Union Controls (6yr connectivity)")
cat("\nDone.\n")

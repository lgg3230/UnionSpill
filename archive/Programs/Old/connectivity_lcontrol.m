...
treatData = readtable("/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/lorenzo_control.csv");
...
for y = start_year:(end_year - 1)
    for nextYear = (y+1):end_year
        ...
        isTreated(treat_idx > 0) = treatData.lorenzo_control(treat_idx(treat_idx > 0));
        ...
        connVarName = sprintf('Connectivity_lcontrol_%d_%d', y, nextYear);
        outVarName  = sprintf('OutflowTreated_lcontrol_%d_%d', y, nextYear);
        inVarName   = sprintf('InflowTreated_lcontrol_%d_%d', y, nextYear);
...
% This adds the treatment dummy (lorenzo_control) to the final table.
mergedTable = outerjoin(finalTable, treatData, 'MergeKeys', true, 'Type', 'full');
% Assume missing treatment status equals 0.
missingTreat = isnan(mergedTable.lorenzo_control);
mergedTable.lorenzo_control(missingTreat) = 0;

%% Save the Final Merged Connectivity Dataset
outputFile = '/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/connectivity_lcontrol_2009_2012.csv';
writetable(mergedTable, outputFile);

disp('Final merged connectivity dataset (for all pairs) with treatment dummy has been saved as connectivity_lcontrol_2009_2012.csv.');
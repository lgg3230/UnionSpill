...
for y = start_year:(end_year - 1)
    nextYear = (y+1);
        ...
        connVarName = sprintf('con_%d_%d', y, nextYear);
        outVarName  = sprintf('outt_%d_%d', y, nextYear);
        outtpVarName = sprintf('outtotreat_p_%d_%d',y,nextYear);
        inVarName   = sprintf('int_%d_%d', y, nextYear);
        inftpVarName = sprintf('infromtreat_p_%d_%d', y, nextYear);
...
outputFile = '/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/connectivity_2009_2017.csv';
writetable(mergedTable, outputFile);

disp('Final merged connectivity dataset (for all pairs) with treatment dummy has been saved as connectivity_2009_2017.csv.');
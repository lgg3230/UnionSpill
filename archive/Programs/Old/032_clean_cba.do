...
drop if valid==0 // only keep legaly valid agreements (67% of original database, drops 308.8k cba's)


* step 0 : keep only cba's with start, file and end dates
drop if missing(start_mdy)|missing(end_mdy)|missing(file_mdy) // no more obs excluded
...
// drops weird cases were dates do not make sense:

drop if file_date_stata<start_date_stata // drops 23.7k obs (3.8%)
drop if file_date_stata>end_date_stata // drops 7.7k obs (1.2%)
...
********************************************************************************
 ...
 duplicates drop // remove duplicate lines . !!! 517k obs dropped!!!!
 ...
 // !! DIFFERENCE WITH 31 DOFILE: MERGING ALL ESTAB ID'S ACROSS ALL YEARS IN THE SAMPLE, INSTEAD OF RESTRICTING TO THE START YEAR.
 // IDK EXACTLY WHY I USED START YEAR IN THE BEGINNING, BUT MAKES MORE SENSE THIS WAY, 
 
 
 use "$cba_dir/cba_firm_exploded_mun.dta",clear
 joinby identificad_8 municipio  using "$rais_aux/rais_unique_estab_09_16.dta" // joinby performs all the interactions between datasets that have the same municipality and firm id
 save "$cba_dir/cba_estab_firm_mun.dta",replace // joinby also drops everyone who was not matched,
 
 use "$cba_dir/cba_firm_exploded_sta.dta",clear
 joinby identificad_8 state  using "$rais_aux/rais_unique_estab_09_16.dta"
 save "$cba_dir/cba_estab_firm_sta.dta",replace
 
 use "$cba_dir/cba_firm_exploded_nat.dta",clear
 joinby identificad_8 using "$rais_aux/rais_unique_estab_09_16.dta"
 save "$cba_dir/cba_estab_firm_nat.dta",replace
 
 use "$cba_dir/cba_estab_firm_mun.dta", clear
 append using "$cba_dir/cba_estab_firm_sta.dta"
 append using "$cba_dir/cba_estab_firm_nat.dta"
 
 gen firm_cba=1 // generates dummies to marke wheter this is a firm level cba
 gen sector_cba=0
 save "$cba_dir/cba_estab_firm.dta", replace
 
 erase "$cba_dir/cba_estab_firm_mun.dta" // erases unnecessary data for hd space conservation.
 erase "$cba_dir/cba_estab_firm_sta.dta"
 erase "$cba_dir/cba_estab_firm_nat.dta"
...
// Here, we have that all data on the master can be found in the using, but not the other way around

// the reason for this is that we drop observations with missing municipality after exploding in before constructing the master dataset. Thus, there are around 187k obs in the using who cant be matched to the master dataset  
...
use "$cba_dir/cba_estab_firm.dta",clear // 3.7M observations 
...
count // so far, we have 3.5 M obs

collapse (first) contract_id-end_year, by(group_3) 
count // goes to 2.5M obs
...
// Drops cba's with weird dates:

drop if file_date_stata>end_date_stata // drops
drop if file_date_stata<start_date_stata // drops 
...
replace treat_ultra=1 if file_date_stata<=mdy(9,14,2012) & end_date_stata>=mdy(9,15,2012) // this only marks estabs with active cbas in 2012, does not mark the cross-section of firms that get treated at some point. "If an agreement was reached before Sep 2012"
...
* Save Final Dataset
save "$cba_dir/collapsed_cba_firm_2.dta", replace

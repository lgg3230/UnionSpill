...
for y = start_year:(end_year - 1)
    nextYear = (y+1);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        filename = sprintf('/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/employers_lsample_%d_%d.csv', y, nextYear);
        ...
        outtotreat_p = 100 * (outToTreated ./ totalOut);
        outtotreat_p(totalOut==0) = NaN;
        
        infromtreat_p = 100 * (inFromTreated ./ totalIn);
        infromtreat_p(totalIn==0) = NaN;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        T_pair = table(unique_emps, connectivityPercentage, outToTreated, inFromTreated, outtotreat_p, infromtreat_p, ...
            'VariableNames', {'identificad', 'Connectivity_Percentage', 'OutflowTreated', 'InflowTreated', 'outtotreat_p','infromtreat_p'});
        
        % Rename the connectivity variables to include the year pair information.
        connVarName = sprintf('con_r_%d_%d', y, nextYear);
        outVarName  = sprintf('outt_r_%d_%d', y, nextYear);
        outtpVarName = sprintf('outtotreat_r_p_%d_%d',y,nextYear);
        inVarName   = sprintf('int_r_%d_%d', y, nextYear);
        inftpVarName = sprintf('infromtreat_r_p_%d_%d', y, nextYear);
        ...
        T_pair.Properties.VariableNames{'outtotreat_p'} = outtpVarName;
        T_pair.Properties.VariableNames{'infromtreat_p'} = inftpVarName;
        
        % Store this table in our cell array.
        connTables{end+1} = T_pair;
        
        fprintf('Processed connectivity measures with treated firms restricted to lorenzo sample for year pair %d-%d.\n', y, nextYear);
end
...
outputFile = '/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/connectivity_restricted_2009_2017.csv';
writetable(mergedTable, outputFile);

disp('Final merged connectivity dataset (for all pairs) with treatment dummy has been saved as connectivity_restricted_2009_2017.csv.');
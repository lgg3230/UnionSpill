...
foreach stat in n_estab n_workers prop_treated mean_emp mean_wage mean_med_wage med_wage ///
                ...
                prop_bin1 prop_bin2 prop_bin3 prop_bin4 {
    forvalues g = 1/5 {
        local `stat'_g`g' = .
    }
}
...
forvalues g = 1/5 {
    ...
    * --------------------------
    * Firm size bins: 1-10, 11-30, 31-100, 101+
    * --------------------------
    count if `cond' & year==2009
    local N_g = r(N)

    count if `cond' & year==2009 & firm_emp>=1 & firm_emp<=10
    local prop_bin1_g`g' = r(N) / `N_g'

    count if `cond' & year==2009 & firm_emp>=11 & firm_emp<=30
    local prop_bin2_g`g' = r(N) / `N_g'

    count if `cond' & year==2009 & firm_emp>=31 & firm_emp<=100
    local prop_bin3_g`g' = r(N) / `N_g'

    count if `cond' & year==2009 & firm_emp>=101 & !missing(firm_emp)
    local prop_bin4_g`g' = r(N) / `N_g'

    di as text "Share 1-10:   " %9.4f `prop_bin1_g`g''
    di as text "Share 11-30:  " %9.4f `prop_bin2_g`g''
    di as text "Share 31-100: " %9.4f `prop_bin3_g`g''
    di as text "Share 101+:   " %9.4f `prop_bin4_g`g''
...
* Rows: Firm size bin shares
file write `fh' "Share 1-10 emp"
forvalues g = 1/5 {
    file write `fh' ";" %9.4f (`prop_bin1_g`g'')
}
file write `fh' _n

file write `fh' "Share 11-30 emp"
forvalues g = 1/5 {
    file write `fh' ";" %9.4f (`prop_bin2_g`g'')
}
file write `fh' _n

file write `fh' "Share 31-100 emp"
forvalues g = 1/5 {
    file write `fh' ";" %9.4f (`prop_bin3_g`g'')
}
file write `fh' _n

file write `fh' "Share 101+ emp"
forvalues g = 1/5 {
    file write `fh' ";" %9.4f (`prop_bin4_g`g'')
}
file write `fh' _n

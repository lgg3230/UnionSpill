#!/usr/bin/env python
"""
generate_cba_period_v2_latex.py

Three-arm comparison table for the CBA periodization trial.

Reads : Tables/max_clause_row/cba_period_v2_comparison.csv   (written by Stata)
Writes: Tables/max_clause_row/cba_period_v2_comparison.tex
        Tables/max_clause_row/cba_period_v2_standalone.tex
...
csv_path = pipeline_dir / "cba_period_v2_comparison.csv"
tex_path = pipeline_dir / "cba_period_v2_comparison.tex"
standalone_path = pipeline_dir / "cba_period_v2_standalone.tex"

ARMS = ["v1_all", "v1_max", "v2"]
...
def build_table(w):
    header = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{CBA Outcomes under Three Definitions of the CBA Period}",
        r"\label{tab:cba_period_v2}",
        r"\scriptsize",
        r"\begin{tabular}{lccccccccc}",
        r"\toprule\toprule",
        r" & \multicolumn{3}{c}{Estimate} & \multicolumn{3}{c}{Observations} "
        r"& \multicolumn{3}{c}{Establishments} \\",
        r"\cmidrule(lr){2-4} \cmidrule(lr){5-7} \cmidrule(lr){8-10}",
        r" & \shortstack{Current}  & \shortstack{Max\\clause} & \shortstack{Period\\v2}"
        r" & \shortstack{Current}  & \shortstack{Max\\clause} & \shortstack{Period\\v2}"
        r" & \shortstack{Current}  & \shortstack{Max\\clause} & \shortstack{Period\\v2} \\",
        r" & (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) & (9) \\",
        r"\midrule",
    ...
    notes = (
        ...
        r"highest-clause row within each establishment by period cell. Column (3) uses "
        ...
        r"establishment's previous year, and indexed by calendar year. Under column (3) "
        ...
        r"and are held fixed across the three columns, so the columns differ only in the "
...
\begin{document}
\pagestyle{empty}
\input{cba_period_v2_comparison.tex}
\end{document}
"""

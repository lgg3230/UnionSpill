********************************************************************************
* cba_period_arms_lagos.do
*
* Same four arms as cba_period_arms.do, but with LAGOS CONTROLS ONLY.
*
* The quartile-bin controls are dropped from every specification:
*   ib0.<outcome>_pre4#i.<period>          (pre-treatment outcome quartile)
*   ib0.l_firm_emp_pre4#i.<period>         (pre-treatment firm size quartile)
*   ib0.totalflows_pw_pre_07_114#i.<period> (pre-treatment per-worker flows quartile)
*
* What remains is Lagos (2026) equation (2): establishment fixed effects plus
* period fixed effects interacted with detailed industry, microregion, and
* negotiation month. Dropping the bins changes which observations are singletons,
* so sample sizes will differ from the controlled table.
*
...
log using "$logs/max_clause_row/cba_period_arms_lagos_`d'_`t'.log", replace text
...
foreach arm in v1_all v1_max v1_mean v2 {
	...
	* extra_cba (flows quartile) intentionally NOT used in this Lagos-controls table
...
use `pf_data', clear
export delimited using "$tables/max_clause_row/cba_period_arms_lagos_comparison.csv", replace

di as result "=== v1_all spillover numb_clauses (LAGOS CONTROLS — will NOT match the published controlled table) ==="
di as result "Controlled-spec reference was 0.0227 (0.1175), 19,693 obs, 4,142 estabs"
list if regression == "spillover" & outcome == "numb_clauses", noobs

di "Finished: `c(current_date)' `c(current_time)'"
capture log close

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "cba_period_arms LAGOS done" "four arms, Lagos controls only"

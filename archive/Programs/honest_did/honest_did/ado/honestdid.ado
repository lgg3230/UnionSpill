...
program honestdid, sclass
    version 14.1
    * PATCHED (UnionSpill): the prebuilt OSQP plugin requires GLIBC_2.34, newer
    * than the cluster's glibc (el8 = 2.28), so it cannot load. OSQP is only used
    * by the FLCI method (flci.mata); the C-LF / Conditional methods used in this
    * project run on ECOS, which loads fine. So we warn instead of exiting.
    * NEVER request method(FLCI) on this cluster -- it will call OSQP and crash.
    cap plugin call honestosqp_plugin, _plugin_check
    if ( _rc ) {
        disp as txt "{bf:note:} OSQP plugin unavailable (GLIBC too old); FLCI disabled, using ECOS-based methods only"
    }

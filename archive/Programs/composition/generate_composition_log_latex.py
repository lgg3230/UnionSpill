...
log-outcome specification.

Two tables: direct effects (Panels A/B/C) and spillover effects.
Outcomes: log versions of tenure, demographics, education shares, HS-or-above,
and average age (8 columns total).

Output: UnionSpill/Tables/composition/composition_log_tables.tex
"""

import re
from pathlib import Path

SPEC = "composition_log"
...
def notes_direct(outcome_desc):
    return (
        ...
        r"Outcomes are in logs; establishments where the raw share equals zero "
        r"in a given year are excluded from that regression automatically. "
        ...
        r"Mean (2011) is the unweighted mean of the log outcome for untreated "
...
def notes_spill(outcome_desc):
    return (
        ...
        r"to treated firms. Outcomes are in logs; establishments where the raw "
        r"share equals zero in a given year are excluded automatically. "
        ...
        _CONTROLS + r" "
        r"Mean (2011) is the unweighted mean of the log outcome for the spillover "
        r"sample in 2011. "
        r"Standard errors clustered at the establishment level in parentheses. " +
        _STARS
    )
...
OUTCOME_GROUPS = [
    dict(
        ...
        outcomes=[
            "l_avg_tenure", "l_male_prop", "l_white_prop",
            "l_prop_nhs", "l_prop_hs", "l_prop_sup", "l_prop_hs_plus",
            "l_avg_age",
        ],
        col_headers=[
            r"Log\\Tenure", r"Log\\Male Sh.", r"Log\\White Sh.",
            r"Log No\\High Sch.", r"Log\\High Sch.", r"Log\\College+",
            r"Log HS\\or Above",
            r"Log\\Avg. Age",
        ],
        direct_notes=notes_direct(
            r"log workforce composition: log average tenure (months), log share "
            r"male, log share white, log share with no high school, log share "
            r"with completed high school, log share with college degree or higher, "
            r"log share with at least a high school diploma, "
            r"and log average worker age"
        ),
        spill_notes=notes_spill(
            r"log workforce composition: log average tenure, log share male, "
            r"log share white, log share with no high school, log share with "
            r"completed high school, log share with college degree or higher, "
            r"log share with at least a high school diploma, "
            r"and log average worker age"
        ),
    ),
...
def main():
    ...
    output_file  = pipeline_dir / "composition_log_tables.tex"

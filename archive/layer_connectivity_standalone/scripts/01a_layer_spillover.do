********************************************************************************
* UNION SPILLOVERS — LAYER-LEVEL SPILLOVER EFFECTS (ABOVE-MEDIAN FIRM EMPLOYMENT)
* Purpose: Same as 07b_layer_spillover.do but restricted to firms whose
*          average total pre-treatment employment (2009-2011) is above the
*          median across layer-balanced-panel firms in the Lagos sample.
*
*          Unlike 07c (which cuts on per-layer employment), the restriction
*          here is a single firm-level size cut: the firm's December employment
*          averaged over 2009-2011 must exceed the median across untreated
*          balanced-panel firms in the Lagos sample. All layers of qualifying
*          firms are kept.
*
...
*
* Output:  Tables/layer_connectivity/results_spill_layer_{edu2,gender,race}_abvmed_firm_layer_spill.csv
*          Tables/layer_connectivity/results_spill_layer_cross_{edu2,gender,race}_abvmed_firm_layer_spill.csv
*          Tables/layer_connectivity/results_spill_firmrestr_{edu2,gender,race}_abvmed_firm_layer_spill.csv
*          Graphs/layer_connectivity/es_{outcome}_abvmed_firm_{spill,cross,firmrestr}_{layer}_{date}.pdf
********************************************************************************
...
foreach layer in edu2 gender race {
	...
	* Restricted set here = layer-balanced panel AND above-median firm employment.
	****************************************************************************

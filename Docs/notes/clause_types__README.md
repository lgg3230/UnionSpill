...
- Direct-effects panels: the same A/B/C samples used in `3012_pct_tfpw.do`.
...
- Specification: same clause-count design as `Programs/3012_pct_tfpw.do`, using `cba_period`, `post_treat_cba`, `pre_treat_cba`, and `totalflows_pw_pre_07_114`.
...
- `3032_clause_types.do`: estimation script for the clause-count and clause-share outcomes.
...
1. Modify the grouped clause-count definitions in `3032_clause_types.do` if you want different clause partitions.
...
- `Programs/cba_similarity/`: separate pipeline that compares pre- vs. post-reform CBA content vectors (cosine, Bray-Curtis, total variation, Jaccard) between untreated firms and a connected/average treated reference. Mirrors the panel spec in `3032_clause_types.do`.

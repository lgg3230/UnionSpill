...
| D3 | **Missing per-worker-flow data silently pooled into the bottom quartile.** `Main_Results_pct_tfpw_07_11.do:216`: `replace totalflows_pw_pre_07_114 = 0 if missing(...)`. Undisclosed in every table note, and contrary to the project's own no-zero-fill rule. The zero-connectivity controls are exactly the small, low-flow firms most likely to be missing **[V]** | Code |
...
| D7 | **2012 coded as post** despite a 25 September ruling and December outcomes — roughly one quarter of exposure **[V]** | `Main_Results_pct_tfpw_07_11.do:114` |

...
|-------|---------|---------------|
| 1, 2 Direct + Spillover | `Main_Results_pct_tfpw_07_11.do` | no mean row — add |
| 3 Clause counts/props | `clause_types/clause_types.do` | no mean row — add |
| 3 CBA value (col 7) | `cba_value/Main_Results_cba_value.do` | has `mean_pre` — **redefine** to `e(sample)` |
| 4 Union confounders | `robustness/Main_Results_union_controls.do` | no mean row — add |
| 5 cols 2–3 | `robustness/Main_Results_robustness_bins.do` | no mean row — add |
| 5 col 4 | `robustness/Main_Results_demo_controls.do` | no mean row — add |
| 5 cols 5–8 | `robustness/Main_Results_micro_ind_q.do` | has `mean_pre` — **redefine** |
| 6 Turnover | `turnover/Main_Results_turnover.do` | has `mean_pre`, **control-group only** — redefine |
| 7 Composition | `composition/Main_Results_composition.do` | has `mean_pre`, **control-group only** — redefine |
| 8 Residualized | `residuals/Main_Results_mincer.do` | has `mean_pre` — redefine |
...
Update (8): `generate_cba_value_latex.py`, `generate_rob_logwages_8col.py` (flip
`INCLUDE_MEAN` default to on), `generate_turnover_latex.py`,
`generate_composition_latex.py`, `generate_mincer_latex.py`,
`generate_resid_replication_tables.py`, `generate_union_controls_latex.py`,
`07_within_firm/02b_make_tables_all.py`.

Write (4), closing the missing-generator gap:
- `Programs/main_results/generate_direct_replication_table.py` → `t_direct.tex`
- `Programs/main_results/generate_spill_replication_table.py` → `t_spill.tex`
- `Programs/clause_types/generate_clause_replication_table.py` → `t_clause.tex`
- `Programs/robustness/generate_union_replication_table.py` → `t_union{,_hw}.tex`
...
document (0.0050, 0.0043). It is left untouched — `Main_Results_union_controls.do`
...
Inlining: `07_within_firm/05_inline_into_replication.py` currently handles only its
...
|---|---|---|
| `main_results/generate_spill_replication_table.py` | `t_spill.tex` | committed fragment — identical |
| `robustness/generate_union_replication_table.py` | `t_union.tex`, `t_union_hw.tex` | document lines 251–292 and 809–850 — both identical |
...
1. **`t_direct` equality row has no reproducible source.** Panel mapping resolved
   ...
   `conn_margins/direct_sample_coef_test.do`, runs against the **legacy** panel and
...
Revised re-run list adds: `cba_value` (via `_run_cba_value_cc.do`) and
`conn_margins/direct_sample_coef_test.do` (currentconn, 4 outcomes).
...
| do-file | Blocks | Note |
|---|---|---|
| `Main_Results_pct_tfpw_07_11.do` | 4 | direct continuous, direct clauses, spill continuous, spill clauses |
| `composition/Main_Results_composition.do` | 2 | replaced firm-average-at-2009 construction |
| `turnover/Main_Results_turnover.do` | 2 | replaced `s_use`-conditioned mean that ignored singleton drops |
| `robustness/Main_Results_union_controls.do` | 6 | patched programmatically; **only 6 of 10 columns exist in code** |
| `robustness/Main_Results_micro_ind_q.do` | 1 | replaced firm-average-at-2009 construction |

**GAP 3 — union columns 7–10 have no source code.** The tracked
`Main_Results_union_controls.do` produces six columns (baseline, union x year FE,
...
`_run_micro_ind_q_hw.do` pattern, with defaults preserving the original monthly
behaviour. Added `_run_union_controls_cc.do` and `_run_union_controls_hw_cc.do`;
...
**Unrelated observation, not acted on.** `turnover/Main_Results_turnover.do` builds
...
**Do-files patched (10 of 14):** `Main_Results_pct_tfpw_07_11.do` (4 blocks),
...
- **Six missing currentconn wrappers written**: `_run_union_controls_cc.do`,
  `_run_union_controls_hw_cc.do`, `_run_robustness_cc.do`, `_run_demo_controls_cc.do`,
  `_run_clause_types_cc.do`, `_run_currentconn_mincer_full.do`,
  `_run_direct_sample_coef_test_cc.do`.
- **Four new generators** for fragments that had none: `generate_direct_replication_table.py`,
  `generate_spill_replication_table.py`, `generate_clause_replication_table.py`,
  `generate_union_replication_table.py`, plus `generate_twopanel_replication_tables.py`
...
`05_inline_into_replication.py` substituted with `count=1`. That was correct for
...
3. `Main_Results_turnover.do` includes `extra_year` in `absorb` for

...
- Verify `Tables/` contains at least one `.tex` file with actual numbers
- Verify `Graphs/` contains at least one `.pdf` or `.png` figure
- If either is empty: **STOP.** Report: "Cannot draft Results — no output files found in Tables/ or Graphs/. Run `/analyze` first, or point me to existing results."
...
1. Read every `.tex` file in `Tables/`
...
- `UnionSpill-paper/Draft.tex` — main document
- `UnionSpill-paper/*.tex` — section files

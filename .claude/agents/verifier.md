...
- Note: `UnionSpill-paper/latexmkrc` configures XeLaTeX, TEXINPUTS, BIBINPUTS

### 2. Script Execution
```bash
Rscript Programs/FILENAME.R 2>&1 | tail -20
```
- Check exit code
...
- Every referenced figure in `Graphs/` exists
...
2. Use `latexmk` for compilation — `UnionSpill-paper/latexmkrc` handles TEXINPUTS and BIBINPUTS

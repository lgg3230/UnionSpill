...
Blocks Write and Edit operations on files outside the specified directories. Use when reviewing code (freeze everything except notes), when writing (freeze scripts), or when editing data pipelines (freeze UnionSpill-paper/).

## Usage

```
/freeze UnionSpill-paper/          # Only allow edits in UnionSpill-paper/
/freeze Programs/ data/  # Only allow edits in Programs/ and data/
...
```json
{
  "freeze": {
    "active": true,
    "allowed_paths": ["UnionSpill-paper/", "Programs/"],

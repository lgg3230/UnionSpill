...
For talks:
```bash
cd UnionSpill-paper/talks && latexmk [file]
```

Note: `UnionSpill-paper/latexmkrc` configures XeLaTeX, TEXINPUTS, and BIBINPUTS. Falls back to manual 3-pass if latexmk is unavailable.

### `/tools validate-bib` — Bibliography Validation
Cross-reference all \cite{} keys in paper and talk files against UnionSpill-paper/bib.bib.
...
- **Single file:** `/tools lint Programs/02_estimate.R`
- **Directory:** `/tools lint Programs/` (recursive)
- **Default:** `/tools lint` (lints `Programs/`)
...
**When to use:**
- Before `/paper-review --code` — catches mechanical violations instantly
...
Step 4: DO NOT TOUCH
  - UnionSpill-paper/, Programs/, data/, explorations/, quality_reports/
  - CLAUDE.md, UnionSpill-paper/bib.bib, README.md, .gitignore

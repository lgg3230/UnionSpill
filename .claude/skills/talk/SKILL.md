...
- **Paper path** (optional): defaults to `UnionSpill-paper/Draft.tex`
...
Save to `UnionSpill-paper/quarto/[format]_talk.qmd` (Quarto, default) or `UnionSpill-paper/talks/[format]_talk.tex` (Beamer).
...
For Quarto:
```bash
cd UnionSpill-paper/quarto && quarto render [file]
```

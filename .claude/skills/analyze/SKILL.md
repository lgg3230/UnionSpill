...
- Stage 3: Publication-ready output (tables to `Tables/`, figures to `Graphs/`)
- Produce `results_summary.md` with all estimates, SEs, and key statistics (MANDATORY)
- Save scripts to `Programs/` (or appropriate language directory)
...
```bash
python3 .claude/scripts/generate_html_report.py code-audit quality_reports/[script]_code_review.md
python3 .claude/scripts/generate_dashboard.py
```
...
3. **Output files** — tables in `Tables/`, figures in `Graphs/`
...
dir.create("UnionSpill-paper/tables", recursive = TRUE, showWarnings = FALSE)
dir.create("UnionSpill-paper/figures", recursive = TRUE, showWarnings = FALSE)
...
# 5. Export ----
# saveRDS(model_fit, "Programs/output/model_fit.rds")
# saveRDS(main_results, "Programs/output/main_results.rds")
```
...
1. **Data-engineer** runs once — language-agnostic cleaning, saves to `Data/`

...
**Agent:** Writer
**Output:** LaTeX section file in UnionSpill-paper/
...
1. Read existing paper draft in `UnionSpill-paper/` (if it exists)
...
5. Check `UnionSpill-paper/bib.bib` for available citations
6. Scan `Tables/` and `Graphs/` for generated output
...
Dispatch Writer with paper type and argument-move templates for the target section. The writer drafts using paragraph types (motivation, result statement, mechanism, etc.), applies design-specific moves, then runs the cleanup pass. Save to `UnionSpill-paper/[section].tex`.

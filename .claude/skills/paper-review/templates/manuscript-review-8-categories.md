...
- All cited tables/figures exist in `Tables/` and `Graphs/`? -5 per missing
...
When invoked via `/paper-review [file.tex]` or `/paper-review --proofread`, run categories **4, 5, 6, 8 only** (writing quality + LaTeX + compilation + notation). No strategy alignment -- just prose and format quality.

When invoked via `/paper-review --all` or `/paper-review --peer`, run all 8 categories.

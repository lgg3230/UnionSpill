...
* cba_period_v2.do
*
* Trial run of an alternative CBA periodization that is unique by construction.
*
...
*
* Output: Tables/max_clause_row/cba_period_v2_comparison.csv
********************************************************************************
...
log using "$logs/max_clause_row/cba_period_v2_`d'_`t'.log", replace text
...
foreach arm in v1_all v1_max v2 {
...
use `pf_data', clear
export delimited using "$tables/max_clause_row/cba_period_v2_comparison.csv", replace

di as result "=== Baseline validation (v1_all spillover numb_clauses) ==="
di as result "Target: 0.0227 (0.1175), 19,693 obs, 4,142 estabs"
list if regression == "spillover" & outcome == "numb_clauses" & arm == "v1_all", noobs

di "Finished: `c(current_date)' `c(current_time)'"
capture log close

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "cba_period_v2 done" "three arms: v1_all, v1_max, v2"

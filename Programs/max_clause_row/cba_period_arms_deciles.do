...
log using "$logs/max_clause_row/cba_period_arms_deciles_`d'_`t'.log", replace text
...
cap drop l_firm_emp_pre10_o
cap drop l_firm_emp_pre10
quietly {
	egen l_firm_emp_pre10_o = cut(l_firm_emp_pre) ///
		if year == 2009 & in_balanced_panel == 1, group(10)
	bys identificad: egen l_firm_emp_pre10 = min(l_firm_emp_pre10_o)
	drop l_firm_emp_pre10_o
}

cap drop totalflows_pw_pre_07_11_10_o
cap drop totalflows_pw_pre_07_11_10
quietly {
	egen totalflows_pw_pre_07_11_10_o = cut(totalflows_pw_pre_07_11) ///
		if year == 2009 & in_balanced_panel == 1, group(10)
	bys identificad: egen totalflows_pw_pre_07_11_10 = min(totalflows_pw_pre_07_11_10_o)
	drop totalflows_pw_pre_07_11_10_o
	replace totalflows_pw_pre_07_11_10 = 0 if missing(totalflows_pw_pre_07_11_10)
}
...
foreach v in numb_clauses wage_clauses emp_clauses other_clauses ///
	...
	cap drop `v'_pre10
	quietly {
		egen `v'_pre10_o = cut(`v'_pre) ///
			if year == 2009 & in_balanced_panel == 1, group(10)
		bys identificad: egen `v'_pre10 = min(`v'_pre10_o)
		drop `v'_pre10_o
	}
}
...
foreach arm in v1_all v1_max v1_mean v2 {
	...
	* period variable and pre/post indicators for this arm
	...
	local extra_cba   "ib0.totalflows_pw_pre_07_11_10#i.`per'"
	...
	* ── DIRECT EFFECTS: numb_clauses, samples A and C ───────────────────────
	foreach panel in A C {
		...
		local absorb_cba "`base_fe_cba' ib0.`outcome'_pre10#i.`per' ib0.l_firm_emp_pre10#i.`per' `extra_cba'"
...
use `pf_data', clear
export delimited using "$tables/max_clause_row/cba_period_arms_deciles_comparison.csv", replace

di as result "=== v1_all spillover numb_clauses (DECILE bins — will NOT match the quartile table) ==="
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "cba_period_arms DECILES done" "four arms, decile control bins"

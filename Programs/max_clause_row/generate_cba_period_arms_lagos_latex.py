#!/usr/bin/env python
"""
generate_cba_period_arms_lagos_latex.py

Four-arm comparison table, LAGOS CONTROLS ONLY (quartile bins removed).

Reads : Tables/max_clause_row/cba_period_arms_lagos_comparison.csv   (written by Stata)
Writes: Tables/max_clause_row/cba_period_arms_lagos_comparison.tex
        Tables/max_clause_row/cba_period_arms_lagos_standalone.tex
...
csv_path = pipeline_dir / "cba_period_arms_lagos_comparison.csv"
tex_path = pipeline_dir / "cba_period_arms_lagos_comparison.tex"
standalone_path = pipeline_dir / "cba_period_arms_lagos_standalone.tex"
...
def build_table(w):
    header = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{CBA Outcomes under Four Treatments of Duplicated Agreement Rows, Lagos Controls Only}",
        r"\label{tab:cba_period_arms_lagos}",
        r"\scriptsize",
    ...
    notes = (
        ...
        r"periodization and in which rows enter. All specifications here include only the "
        r"controls in Lagos (2026) equation (2): establishment fixed effects and "
        r"period fixed effects interacted with two-digit industry, microregion, and "
        r"negotiation-month indicators. The quartile bins of pre-treatment firm size, "
        r"per-worker flows, and the outcome that appear in the paper's main "
        r"specifications are omitted throughout, so sample sizes differ from the "
        r"controlled tables because a different set of observations is a singleton under "
        r"the reduced fixed-effect structure. Sample A "
...
\begin{document}
\pagestyle{empty}
\input{cba_period_arms_lagos_comparison.tex}
\end{document}
"""

...
* cba_period_v3.do
*
* Five arms, one Stata process. Adds cba_period_v3 to the previous four.
*
* cba_period_v3: pre-treatment periods 1 and 2 are the same agreements as ever
* (2009 agreement; earliest renewal, the omitted category), deduplicated to the
* earliest December. From 2012 onward the period is simply the CALENDAR YEAR,
* using whichever agreement is in force that December:
*     2012 -> 3, 2013 -> 4, 2014 -> 5, 2015 -> 6, 2016 -> 7
* Unlike v2, continuation years are RETAINED — v2 keeps only the December in
* which a new agreement first takes effect, v3 keeps every December and lets the
* year define the period. Uniqueness is automatic: the panel holds one row per
* establishment-year, so one row per establishment-period.
*
...
log using "$logs/max_clause_row/cba_period_v3_`d'_`t'.log", replace text
...
* ── v3: period = calendar year from 2012 onward ─────────────────────────────
* Pre-treatment identical to v1/v2; post-treatment is simply the December of
* each calendar year, whatever agreement is in force then.
cap drop cba_period_v3
gen cba_period_v3 = .
replace cba_period_v3 = 1 if avg_file_date == earliest2009_avg - 1 & !missing(avg_file_date)
replace cba_period_v3 = 2 if avg_file_date == second_cba_avg       & !missing(avg_file_date)

replace cba_period_v3 = 3 if year == 2012 & missing(cba_period_v3)
replace cba_period_v3 = 4 if year == 2013 & missing(cba_period_v3)
replace cba_period_v3 = 5 if year == 2014 & missing(cba_period_v3)
replace cba_period_v3 = 6 if year == 2015 & missing(cba_period_v3)
replace cba_period_v3 = 7 if year == 2016 & missing(cba_period_v3)

* Pre-treatment periods 1-2: keep the EARLIEST December (as in v2).
sort identificad cba_period_v3 year
cap drop pre_rank_v3
by identificad cba_period_v3: gen long pre_rank_v3 = _n
count if inlist(cba_period_v3, 1, 2) & pre_rank_v3 > 1
di as result "v3 pre-period rows dropped for uniqueness: " r(N)
replace cba_period_v3 = . if inlist(cba_period_v3, 1, 2) & pre_rank_v3 > 1

gen pre_treat_cba_v3  = cond(cba_period_v3 < 2,  1, 0)
gen post_treat_cba_v3 = cond(cba_period_v3 >= 3, 1, 0) if !missing(cba_period_v3)
...
di as result "=== cba_period_v3 distribution ==="
tab cba_period_v3, missing

preserve
	keep if !missing(cba_period_v3)
	isid identificad cba_period_v3
	di as result "PASSED: isid identificad cba_period_v3"
restore
...
foreach arm in v1_all v1_max v1_mean v2 v3 {
	...
	* period variable and pre/post indicators for this arm
	if "`arm'" == "v2" {
		...
		local per  "cba_period_v3"
		local pre  "pre_treat_cba_v3"
		local post "post_treat_cba_v3"
	}
	else {
	...
	local extra_cba   "ib0.totalflows_pw_pre_07_114#i.`per'"
	...
	* ── DIRECT EFFECTS: numb_clauses, samples A and C ───────────────────────
	foreach panel in A C {
		...
		local absorb_cba "`base_fe_cba' ib0.`outcome'_pre4#i.`per' ib0.l_firm_emp_pre4#i.`per' `extra_cba'"
...
use `pf_data', clear
export delimited using "$tables/max_clause_row/cba_period_v3_comparison.csv", replace

di as result "=== Baseline validation (v1_all spillover numb_clauses) ==="
di as result "Target: 0.0227 (0.1175), 19,693 obs, 4,142 estabs"
list if regression == "spillover" & outcome == "numb_clauses", noobs

di "Finished: `c(current_date)' `c(current_time)'"
capture log close

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "cba_period_v3 done" "five arms incl. v3 (period = calendar year)"

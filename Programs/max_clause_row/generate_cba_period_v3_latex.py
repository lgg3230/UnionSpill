#!/usr/bin/env python
"""
generate_cba_period_v3_latex.py

Five-arm comparison table including cba_period_v3.

Reads : Tables/max_clause_row/cba_period_v3_comparison.csv   (written by Stata)
Writes: Tables/max_clause_row/cba_period_v3_comparison.tex
        Tables/max_clause_row/cba_period_v3_standalone.tex
...
csv_path = pipeline_dir / "cba_period_v3_comparison.csv"
tex_path = pipeline_dir / "cba_period_v3_comparison.tex"
standalone_path = pipeline_dir / "cba_period_v3_standalone.tex"

ARMS = ["v1_all", "v1_max", "v1_mean", "v2", "v3"]
...
def build_table(w):
    header = [
        r"\begin{table}[H]",
        r"\centering",
        r"\caption{CBA Outcomes under Five Treatments of Duplicated Agreement Rows}",
        r"\label{tab:cba_period_v3}",
        r"\scriptsize",
        r"\begin{tabular}{lccccccccccccccc}",
        r"\toprule\toprule",
        r" & \multicolumn{5}{c}{Estimate} & \multicolumn{5}{c}{Observations} "
        r"& \multicolumn{5}{c}{Establishments} \\",
        r"\cmidrule(lr){2-6} \cmidrule(lr){7-11} \cmidrule(lr){12-16}",
        r" & Current & \shortstack{Max\\clause} & \shortstack{Mean\\clause} & \shortstack{Period\\v2} & \shortstack{Period\\v3}"
        r" & Current & \shortstack{Max\\clause} & \shortstack{Mean\\clause} & \shortstack{Period\\v2} & \shortstack{Period\\v3}"
        r" & Current & \shortstack{Max\\clause} & \shortstack{Mean\\clause} & \shortstack{Period\\v2} & \shortstack{Period\\v3} \\",
    ...
    notes = (
        ...
        r"establishment's previous year, and indexed by calendar year. Column (5) keeps the "
        r"same two pre-treatment periods but lets the post-treatment period be simply the "
        r"calendar year from 2012 onward, using whichever agreement is in force that "
        r"December; unlike column (4) it retains continuation years rather than only the "
        r"December in which a new agreement first takes effect. Under columns (4) and (5) "
        ...
        r"and are held fixed across the five columns, so the columns differ only in the "
        ...
        r"industry, microregion, and negotiation-month indicators and with quartile bins "
        r"of pre-treatment firm size, per-worker flows, and the outcome. Sample A "
...
\begin{document}
\pagestyle{empty}
\input{cba_period_v3_comparison.tex}
\end{document}
"""

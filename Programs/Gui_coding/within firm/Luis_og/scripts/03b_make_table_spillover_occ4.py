...
Reads CSVs produced by 03a_layer_spillover_occ4.do and outputs:
  output/table_layer_specs_abvmed_firm_occ4.tex
  output/table_layer_specs_abvmed_firm_occ4.csv
...
LAYER = "occ4"
SPEC  = "abvmed_firm_layer_spill"

SPECS = [
    {
        "label":    "(1) Within-firm FE",
        "sublabel": r"firm$\times$year FE",
        "file_tpl": f"results_spill_layer_{{layer}}_{SPEC}.csv",
        "outcomes": ["lr_remdezr_layer", "l_layer_emp"],
    },
    {
        "label":    "(2) Cross-firm FE",
        "sublabel": r"micro$\times$yr + ind$\times$yr + mode$\times$yr",
        "file_tpl": f"results_spill_layer_cross_{{layer}}_{SPEC}.csv",
        "outcomes": ["lr_remdezr_layer", "l_layer_emp"],
    },
    {
        "label":    "(3) Firm-level (restricted)",
        "sublabel": "firm FE",
        "file_tpl": f"results_spill_firmrestr_{{layer}}_{SPEC}.csv",
        "outcomes": ["lr_remdezr_w", "l_firm_emp"],
    },
]

OUTCOME_ORDER = [
    # (generic label, spec0_outcome, spec1_outcome, spec2_outcome)
...
if missing:
    print("WARNING — missing files (run 03a_layer_spillover_occ4.do first):")
    for f in missing:
        print(f"  {f}")
...
def build_latex() -> str:
    ...
    ncols  = 1 + n_out * n_spec

    col_spec = "l" + "ccc" * n_out
    lines = []
    lines.append(r"\begin{table}[H]")
    lines.append(r"\centering")
    lines.append(r"\caption{Layer spillover effects --- occupation layers (CBO 2002), above-median firm employment}")
    ...
    for _ in OUTCOME_ORDER:
        for spec in SPECS:
            num = spec["label"].split(")")[0].lstrip("(")
            h2.append(r"\shortstack{" + num + r" \\ " + spec["sublabel"] + r"}")
    ...
    lines.append(r"\quad Connectivity $\times$ Post" + " & " + " & ".join(cells_for_rt("main", fmt_coef)[1:]) + r" \\")
    lines.append(" & ".join(cells_for_rt("main_se", fmt_se)) + r" \\")
    lines.append(r"\quad Pre-trend (placebo)" + " & " + " & ".join(cells_for_rt("pre", fmt_coef)[1:]) + r" \\")
    ...
    lines.append(
        ...
        r"The sample is restricted to firms whose average December employment in 2009--2011 is above "
        r"the median across untreated balanced-panel firms in the Lagos sample. "
        r"All regressions are further restricted to untreated firms in the balanced firm panel. "

...
*          results_direct_panelA_turnover_firmscale_ll.csv
*          results_direct_panelB_turnover_firmscale_ll.csv
*          results_direct_panelC_turnover_firmscale_ll.csv
*          results_spill_turnover_firmscale_ll.csv
...
log using "$logs/turnover/FinalResults_turnover_firmscale_ll_`d'_`t'.log", replace text
...
foreach v of global turnover_fs_raw {
	...
	* Firm-scaled outcome
	cap drop fs_`v'
	gen double fs_`v' = `v' / fm_`v' if fm_`v' > 0 & !missing(fm_`v') & `v' > 0
...
local spec       "turnover_firmscale_ll"
...
foreach panel in A B C {
	capture erase "$tables/turnover/results_direct_panel`panel'_turnover_firmscale_ll.csv"
	tempname fh
	file open `fh' using "$tables/turnover/results_direct_panel`panel'_turnover_firmscale_ll.csv", write replace
	file write `fh' "spec,section,outcome,row_type,value" _n
	file close `fh'
}

capture erase "$tables/turnover/results_spill_turnover_firmscale_ll.csv"
tempname fh
file open `fh' using "$tables/turnover/results_spill_turnover_firmscale_ll.csv", write replace
file write `fh' "spec,section,outcome,row_type,value" _n
file close `fh'
...
foreach panel in A B C {
	...
	local csv_out "$tables/turnover/results_direct_panel`panel'_turnover_firmscale_ll.csv"
...
local csv_spill "$tables/turnover/results_spill_turnover_firmscale_ll.csv"
...
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Stata done" "turnover_firmscale_ll regressions complete"

...
log level specification (raw count numerators, not rates).

Output: UnionSpill/Tables/turnover/turnover_loglevel_tables.tex
"""

import re
from pathlib import Path

SPEC = "turnover_loglevel"
...
DIRECT_OUTCOMES = [
    "ll_hired_u", "ll_separations_u", "ll_layoffs_u", "ll_quits_u",
    "ll_churn_count_u", "ll_jan_dec_count", "ll_dec_yoy_count",
    "l_total_hours", "l_firm_emp",
]
DIRECT_HEADERS = [
    r"Log\\Hires", r"Log\\Seps.", r"Log\\Layoffs", r"Log\\Quits",
    r"Log\\Churn", r"Log\\Ret.\\(Jan--Dec)", r"Log\\Ret.\\(YoY)",
...
OUTCOME_GROUPS = [
    dict(
        ...
        direct_notes=notes_direct(
            ...
            r"(= hires + separations). "
            r"Columns~(6)--(7) report log within-year retention count "
            r"(January stayers surviving to December) and log year-over-year "
            ...
            r"Establishments with a zero count in a given year are excluded "
            r"from that outcome's regression",
        ),
...
def main():
    ...
    output_file  = pipeline_dir / "turnover_loglevel_tables.tex"

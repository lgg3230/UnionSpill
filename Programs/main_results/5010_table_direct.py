...
Inputs:
    Tables/pct_tfpw_cc/results_direct_panel{A,C}_tfpw_07_11_pct.csv
        written by 4012_pct_tfpw.do via 4011_pct_tfpw.do
    Tables/conn_margins/direct_sample_coef_test_currentconn.csv
        written by conn_margins/4021_direct_sample_coef_test.do -- the
...
Usage
-----
    python 5010_table_direct.py                # no mean row
    python 5010_table_direct.py --with-mean    # adds it
"""
...
def load_equality(path):
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "Run Programs/conn_margins/4021_direct_sample_coef_test.do first."
        )
    out = {}
    with open(path) as fh:
...
def build(panels, equality, with_mean):
    ...
    if missing:
        raise SystemExit(
            ...
            + "\nRe-run 4021_direct_sample_coef_test.do (it covers all four)."
        )

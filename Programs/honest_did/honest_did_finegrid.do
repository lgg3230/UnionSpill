...
*
* Restrictions:
*   Delta^RM (relative magnitudes) : Mbar grid 0(0.05)2, identical across all 8 (scale-free).
...
*
* Output:
*   Tables/honest_did/honest_did_results_fine.csv  (long: effect,outcome,restriction,m,lb,ub,is_original)
...
program define _hd_one
	...
	* ── Delta^RM (relative magnitudes), fixed fine grid 0(0.05)2 ─────────────
	capture noisily honestdid, b(HDB) vcov(HDV) l_vec(`LV') numpre(`numpre') ///
		delta(rm) mvec(0(0.05)2) method(C-LF)
	if (_rc == 0) {
...
program define _pt_run
	...
	foreach pw in 50 80 {
		...
		if (_rc == 0) {
			...
			tempname fh
			file open `fh' using "$tables/honest_did/pretrends_results_fine.csv", write append
...
program define _hd_writeci
	...
	tempname fh
	file open `fh' using "$tables/honest_did/honest_did_results_fine.csv", write append
	forvalues r = 1/`nr' {
		local m  = `CImat'[`r',1]
		local lb = `CImat'[`r',2]
		local ub = `CImat'[`r',3]
...
capture erase "$tables/honest_did/honest_did_results_fine.csv"
tempname fh
file open `fh' using "$tables/honest_did/honest_did_results_fine.csv", write replace
file write `fh' "effect,outcome,restriction,target,m,lb,ub,is_original" _n
file close `fh'

capture erase "$tables/honest_did/pretrends_results_fine.csv"
tempname fh
file open `fh' using "$tables/honest_did/pretrends_results_fine.csv", write replace

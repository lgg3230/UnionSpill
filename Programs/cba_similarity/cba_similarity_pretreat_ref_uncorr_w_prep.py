...
of their connected treated partners. Uses the ORIGINAL i<j-biased
bilateral_conn_pw weights (no focal-firm denominator correction) — i.e., the
moving-reference uncorrected-weight construction from cba_similarity_prep.py
combined with a fixed period-2 reference. Pair-twin of
cba_similarity_pretreat_ref_prep.py: the only thing that changes between the
two scripts is the weight definition.
...
output_path    = rais_aux / "cba_similarity_pretreat_ref_uncorr_w_panel.dta"
...
# No firm-panel-based pair restriction here: the regression-side `s_spill`
# filter (lagos_sample_avg=1 & in_balanced_panel=1 & treat_ultra=0) handles
# the sample on the focal side; partner-side balanced-panel status is NOT
# imposed, exactly as in the original moving-reference exercise.
...
# Untreated focal → treated partner (CSV may store either orientation; same
# bilateral_conn_pw weight regardless of which side of i<j we sit on).
fwd = bilateral[
...
print(f"  Untreated→treated pairs (original weights): {len(pairs):,}")
...
# Inner-merge on (identificad, cba_period) with cba_similarity_panel.dta so that
# every row in this panel also exists in the moving-reference panel. This makes
# the regression sample identical to cba_similarity_table.tex; the ONLY thing
# that differs across the two tables is whether similarity is measured against
# the moving same-period treated reference or the fixed period-2 reference.
sample_keys_path = rais_aux / "cba_similarity_panel.dta"
...
subprocess.run(
    ["curl", "-s", "-o", "/dev/null",
     "-H", "Title: cba_similarity_pretreat_ref_uncorr_w_prep done",
     "-d", f"Saved {len(result):,} obs to cba_similarity_pretreat_ref_uncorr_w_panel.dta",

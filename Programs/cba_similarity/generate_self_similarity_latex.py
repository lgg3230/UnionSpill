"""
generate_self_similarity_latex.py
MIRROR of generate_similarity_latex.py.

Reads results_spill_self_cba_similarity.csv and produces a LaTeX table with
one column per similarity measure (untreated firms vs. their OWN period-2
CBA — within-firm content stability).
"""
...
csv_path   = tables_dir / "results_spill_self_cba_similarity.csv"
tex_path   = tables_dir / "self_cba_similarity_table.tex"
...
base_spec  = "self_cba_similarity_tfpw_07_11"
union_spec = "self_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{CBA Content Stability: Untreated Firms vs.\ Own Pre-Treatment CBA}",
    r"\label{tab:self_cba_similarity}",
    r"\scriptsize",
    ...
    r"\smallskip",
    (r"\scriptsize\textit{Notes:} This table reports estimates of within-firm "
     r"CBA content stability on connectivity for untreated firms. For each "
     r"untreated firm $i$ and CBA period $t$, the outcome is the similarity "
     r"between firm $i$'s clause vector at period $t$ and firm $i$'s OWN "
     r"clause vector at \textbf{cba\_period $= 2$} (the last pre-Sumula 277 CBA). "
     r"By construction, similarity equals 1 at period 2; the regression captures "
     r"how much of that stability is broken in subsequent periods, and whether "
     r"the break correlates with connectivity to treated firms. "
     r"Connectivity is scaled to 1 at the 90th percentile of the spillover sample "
     r"at year 2009. Panel A includes establishment, industry $\times$ period, "

********************************************************************************
* treated_cba_self_similarity.do
*
* MIRROR of treated_cba_similarity.do, with the similarity outcome computed
* against the firm's OWN period-2 CBA (within-firm content stability).
*
* For each (treated firm i, cba_period t):
*   sim_{it} = sim(x_{it}, x_{i,2})
* The hypothesis: treated firms with high connectivity drift further from
* their own pre-treatment CBAs after Sumula 277, so the connectivity-by-post
* interaction should be negative.
*
* Pipeline:
*   1. Load + setup variables (mirrors treated_cba_similarity.do)
*   2. Export intermediate clause data (cba_clauses_by_period_treated.dta)
*   3. Python computes self-similarities (treated_cba_self_similarity_prep.py)
*   4. Merge similarity panel and run panel regressions
*
* Output:
*   Tables/cba_similarity/results_spill_treated_self_cba_similarity.csv
*   Tables/cba_similarity/results_spill_treated_self_ln_cba_similarity.csv
*   Tables/cba_similarity/treated_self_cba_similarity_table.tex
*   Tables/cba_similarity/treated_self_ln_cba_similarity_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/treated_cba_self_similarity_`d'_`t'.log", replace text
...
* lagos-sample untreated firms (to match the existing treated_cba_similarity.do
* sample structure; the prep computes self-similarity for treated firms only).
...
********************************************************************************
* SECTION 3: PRE-TREATMENT CONTROLS
********************************************************************************
...
di as result "Running Python self-similarity prep (treated)..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/treated_cba_self_similarity_prep.py"
...
merge m:1 identificad cba_period using "$rais_aux/treated_cba_self_similarity_panel.dta", ///
    keep(master match) gen(m_sim)
drop m_sim

* Connectivity to untreated/control (firm-level, time-invariant)
preserve
...
local spec              "treated_self_cba_similarity_tfpw_07_11"
...
capture erase "$tables/cba_similarity_log/results_spill_treated_self_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_treated_self_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_treated_self_ln_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_treated_self_ln_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

********************************************************************************
* SECTION 10: TREATED SELF-SIMILARITY — LEVELS
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "TREATED SELF-SIMILARITY (LEVELS)"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/cba_similarity_log/results_spill_treated_self_cba_similarity.csv"
...
foreach fe_variant in base union {
    ...
    foreach outcome of local sim_outcomes {
        ...
        di as text "  Estimating: `outcome' (treated self, `fe_variant')"
...
********************************************************************************
* SECTION 11: TREATED SELF-SIMILARITY — LOG
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "TREATED SELF-SIMILARITY — LOG"
di as result "-----------------------------------------------------------------------"

local spec_ln "treated_self_ln_cba_similarity_tfpw_07_11"
local csv_ln  "$tables/cba_similarity_log/results_spill_treated_self_ln_cba_similarity.csv"
...
foreach fe_variant in base union {
    ...
    foreach outcome of local ln_sim_outcomes {
        ...
        di as text "  Estimating: `outcome' (treated self, log, `fe_variant')"
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_treated_self_similarity_latex.py"
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_treated_self_ln_similarity_latex_log.py"

shell source "$programs/notify.sh" && notify "treated_cba_self_similarity done (log1p)" "Treated self-similarity regressions and tables complete"

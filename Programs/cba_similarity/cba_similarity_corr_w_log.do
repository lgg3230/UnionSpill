********************************************************************************
* cba_similarity_corr_w.do
*
* MIRROR of cba_similarity.do with ORIENTATION-CORRECTED connectivity weights.
* Reference vector is still per-period (moving), so this isolates the effect
* of swapping the weight definition from bilateral_conn_pw (smaller-ID
* denominator) to focal-firm-corrected per-worker connectivity.
*
* For each (untreated firm i, cba_period t):
*   sim_{it} = sim(x_{it}, Σ_j w_{ij}*x_{jt} / Σ_j w_{ij})
* j ranges over treated firms with bilateral flows to i; weights w_{ij} are
* the focal-firm-corrected per-worker connectivity (mean over 2007-2011
* year-pairs of flows_t / avg_emp_i_t).
*
* Output:
*   Tables/cba_similarity/results_spill_corr_w_cba_similarity.csv
*   Tables/cba_similarity/results_spill_corr_w_ln_cba_similarity.csv
*   Tables/cba_similarity/corr_w_cba_similarity_table.tex
*   Tables/cba_similarity/corr_w_ln_cba_similarity_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/cba_similarity_corr_w_`d'_`t'.log", replace text
...
********************************************************************************
* SECTION 4: RUN PYTHON PREP SCRIPT (orientation-corrected weights)
********************************************************************************

di as result "Running Python corr_w similarity prep..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/cba_similarity_corr_w_prep.py"
...
di as result "Merging corr_w similarity measures into panel..."

merge m:1 identificad cba_period using "$rais_aux/cba_similarity_corr_w_panel.dta", ///
	keep(master match) gen(m_sim)
drop m_sim

label var cosine          "Cosine similarity to connected treated avg (corr-w)"
label var bray_curtis     "Bray-Curtis similarity to connected treated avg (corr-w)"
label var total_variation "Total variation similarity to connected treated avg (corr-w)"
label var ruzicka         "Ruzicka similarity to connected treated avg (corr-w)"

local sim_outcomes "cosine bray_curtis total_variation ruzicka"

di as result "Corr_w similarity merge complete. Observations with similarity:"
...
local spec             "corr_w_cba_similarity_tfpw_07_11"
...
capture erase "$tables/cba_similarity_log/results_spill_corr_w_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_corr_w_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_corr_w_ln_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_corr_w_ln_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

********************************************************************************
* SECTION 9: SPILLOVER EFFECTS — LEVELS
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — CORRECTED WEIGHTS"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/cba_similarity_log/results_spill_corr_w_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local sim_outcomes {
		...
		di as text "  Estimating: `outcome' (corr_w, `fe_variant')"
...
********************************************************************************
* SECTION 9b: SPILLOVER EFFECTS — LOG
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS — LOG (CORRECTED WEIGHTS)"
di as result "-----------------------------------------------------------------------"

local spec_ln    "corr_w_ln_cba_similarity_tfpw_07_11"
local csv_ln     "$tables/cba_similarity_log/results_spill_corr_w_ln_cba_similarity.csv"
...
foreach fe_variant in base union {
	...
	foreach outcome of local ln_sim_outcomes {
		...
		di as text "  Estimating: `outcome' (corr_w, log, `fe_variant')"
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_corr_w_similarity_latex.py"
di as result "LaTeX table written to Tables/cba_similarity/corr_w_cba_similarity_table.tex"

shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_corr_w_ln_similarity_latex_log.py"
di as result "LaTeX table written to Tables/cba_similarity/corr_w_ln_cba_similarity_table.tex"

shell source "$programs/notify.sh" && notify "cba_similarity_corr_w done (log1p)" "Corrected-weights similarity regressions and tables complete"

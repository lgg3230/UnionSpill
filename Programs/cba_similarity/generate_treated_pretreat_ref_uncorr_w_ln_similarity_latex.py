"""
generate_treated_pretreat_ref_uncorr_w_ln_similarity_latex.py
MIRROR of generate_treated_ln_similarity_latex.py.

Reads results_spill_treated_pretreat_ref_uncorr_w_ln_cba_similarity.csv and produces a
LaTeX table with one column per log-similarity measure for the treated-side
...
csv_path   = tables_dir / "results_spill_treated_pretreat_ref_uncorr_w_ln_cba_similarity.csv"
tex_path   = tables_dir / "treated_pretreat_ref_uncorr_w_ln_cba_similarity_table.tex"
...
base_spec  = "treated_pretreat_ref_uncorr_w_ln_cba_similarity_tfpw_07_11"
union_spec = "treated_pretreat_ref_uncorr_w_ln_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{Reciprocal CBA Similarity (Log): Treated Firms vs.\ Fixed Pre-Treatment Untreated Reference}",
    r"\label{tab:treated_pretreat_ref_uncorr_w_ln_cba_similarity}",
    r"\scriptsize",
    ...
    (r"\textit{Notes:} This table reports treated-side reciprocal estimates "
     r"of \emph{log} CBA content similarity on connectivity, where the reference "
     r"vector is fixed at the untreated partners' pre-Sumula 277 contracts "
     r"(\textbf{cba\_period $= 2$}). Outcomes are $\ln(\text{sim}_{it})$ for "
     r"$\text{sim} \in \{\text{cosine}, \text{Bray-Curtis}, \text{total variation}, "
     r"\text{Ruzicka}\}$, dropping firm-period observations with zero similarity. "
     r"Sample, weights, and fixed-effects structure are identical to "
     r"Table~\ref{tab:treated_pretreat_ref_uncorr_w_cba_similarity}; see notes there. "

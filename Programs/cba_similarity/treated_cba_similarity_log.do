********************************************************************************
* treated_cba_similarity.do
*
* MIRROR of cba_similarity.do — focused on TREATED firms.
*
* Tests the reciprocal of the spillover content-diffusion result: do treated
* firms whose worker flows reach more untreated firms become more similar (in
* CBA content) to those connected untreated partners after Súmula 277?
*
* For each (treated firm i, cba_period t):
*   sim_{it} = sim(x_{it}, Σ_j w_{ij}*x_{jt} / Σ_j w_{ij})
* where j ranges over UNTREATED firms with bilateral flows to i (w_{ij}).
*
* Connectivity measure: totalcontrol_pw_n (MIRROR of totaltreat_pw_n)
*   For each firm and year-pair (y, y+1):
*     totalcontrol_pw_{y,y+1} = (outflows + inflows to control firms)
*                             / avg(firm_emp_y, firm_emp_{y+1})
*   then averaged across the four 2007-2011 year-pairs (with NaN-aware mean)
*   to give totalcontrol_pw_n. Normalized to its 90th percentile in the
...
*
* Outputs:
*   Tables/cba_similarity/results_spill_treated_cba_similarity.csv
*   Tables/cba_similarity/results_spill_treated_ln_cba_similarity.csv
*   Tables/cba_similarity/treated_cba_similarity_table.tex
*   Tables/cba_similarity/treated_ln_cba_similarity_table.tex
********************************************************************************
...
log using "$logs/cba_similarity_log/treated_cba_similarity_`d'_`t'.log", replace text
...
di as result "Running Python similarity prep (treated mirror)..."
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/treated_cba_similarity_prep.py"
...
merge m:1 identificad cba_period using "$rais_aux/treated_cba_similarity_panel.dta", ///
...
label var cosine          "Cosine similarity to connected untreated avg"
label var bray_curtis     "Bray-Curtis similarity to connected untreated avg"
label var total_variation "Total variation similarity to connected untreated avg"
label var ruzicka         "Ruzicka similarity to connected untreated avg"
...
local spec              "treated_cba_similarity_tfpw_07_11"
...
capture erase "$tables/cba_similarity_log/results_spill_treated_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_treated_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

capture erase "$tables/cba_similarity_log/results_spill_treated_ln_cba_similarity.csv"
tempname fh
file open `fh' using "$tables/cba_similarity_log/results_spill_treated_ln_cba_similarity.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'

********************************************************************************
* SECTION 10: SPILLOVER (RECIPROCAL) EFFECTS — LEVELS
********************************************************************************

di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "RECIPROCAL EFFECTS (TREATED → UNTREATED REFERENCE)"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/cba_similarity_log/results_spill_treated_cba_similarity.csv"
...
foreach fe_variant in base union {
    ...
    foreach outcome of local sim_outcomes {
        ...
        di as text "  Estimating: `outcome' (treated mirror, `fe_variant')"
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "RECIPROCAL EFFECTS — LOG SIMILARITY"
di as result "-----------------------------------------------------------------------"

local spec_ln "treated_ln_cba_similarity_tfpw_07_11"
local csv_ln  "$tables/cba_similarity_log/results_spill_treated_ln_cba_similarity.csv"
...
foreach fe_variant in base union {
    ...
    foreach outcome of local ln_sim_outcomes {
        ...
        di as text "  Estimating: `outcome' (treated mirror, log, `fe_variant')"
...
* [log variant] raw-table generator disabled: shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_treated_similarity_latex.py"
shell ~/.conda/envs/venv_python312/bin/python "$programs/cba_similarity/generate_treated_ln_similarity_latex_log.py"

shell source "$programs/notify.sh" && notify "treated_cba_similarity done (log1p)" "Treated mirror similarity regressions and tables complete"

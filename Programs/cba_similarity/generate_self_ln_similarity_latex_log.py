"""
generate_self_ln_similarity_latex.py
MIRROR of generate_ln_similarity_latex.py.

Reads results_spill_self_ln_cba_similarity.csv and produces a LaTeX table
with one column per log-similarity measure (untreated firms vs. their OWN
period-2 CBA — within-firm content stability).
"""
...
csv_path   = tables_dir / "results_spill_self_ln_cba_similarity.csv"
tex_path   = tables_dir / "self_ln_cba_similarity_table.tex"
...
base_spec  = "self_ln_cba_similarity_tfpw_07_11"
union_spec = "self_ln_cba_similarity_tfpw_07_11_union"
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    r"\caption{Log CBA Content Stability: Untreated Firms vs.\ Own Pre-Treatment CBA}",
    r"\label{tab:self_ln_cba_similarity}",
    r"\scriptsize",
    ...
    r"\smallskip",
    (r"\scriptsize\textit{Notes:} This table reports estimates of \emph{log} "
     r"within-firm CBA content stability on connectivity for untreated firms. "
     r"Outcomes are $\ln(\text{sim}_{it})$ for $\text{sim} \in \{\text{cosine}, "
     r"\text{Bray-Curtis}, \text{total variation}, \text{Ruzicka}\}$, dropping "
     r"firm-period observations with zero similarity. Each similarity is computed "
     r"between firm $i$'s clause vector at period $t$ and firm $i$'s OWN clause "
     r"vector at cba\_period $= 2$. Sample, connectivity, and fixed-effects "
     r"structure are identical to Table~\ref{tab:self_cba_similarity}; "
     r"see notes there. Standard errors clustered at the establishment level. "

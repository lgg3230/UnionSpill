%% Connectivity to Treatment - Post-Treatment Period (2011-2016)
% Computes flows to treated establishments for post-treatment year pairs
% Uses the SAME treatment definition as pre-period (time-invariant)
...
%% Load Lagos Sample Dataset (same treatment definition as pre-period)
% Treatment status is time-invariant
...
%% Loop: compute connectivity matrix and sum flows to treated for every two consecutive years
...
for y = start_year:(end_year-1)
    ...
    % Mark which establishments are treated (using time-invariant definition)
...
outputFile = '/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/connectivity_treat_2011_2016.csv';
writetable(finaltable, outputFile);

fprintf('\nSaved connectivity to treatment data (2011-2016) to %s\n', outputFile);

...
use "$cba_dir/cnes_contracts_coverage_updated.dta", clear // load dataset with coverage info per contract. Not clean. 942k obs
...
while regexm(identificad_cba, "[^0-9]"){ // regexm(identificad_cba, "[^0-9]") searches identificad_cba for non-number characters
 ...
 replace codigo_municipio = "000000" if mode_state == "nacional" & (missing(codigo_municipio) | codigo_municipio=="0")
 ...
 drop if missing(codigo_municipio) // previously there were 6,045 rows with missing municipality. Now there are only 6,029. After update there are only 646
...
drop if start_date_stata>file_date_stata // 23,941
drop if file_date_stata>end_date_stata // 7,738
...
drop if end_year<2009 // drop if cba ends before 2009:1.6k dropped
...
********************************************************************************
 ...
 drop if missing(codigo_municipio) // drop if coverage info is missing: 54,595 ou of 10.6M
...
// Count how many cba's are negotiated (filed) per year per union

bys identificad file_year union_id: gen n_negs_union_year = _N
...
//  END: Seems preety clear that end date is the appropriate one for cba's ending before the ultractivity reform.
//  As pointed out both in the reading of the reform text and in Lagos's email, the cba's would only end with a 
//  new negotiation after the reform, so their effective end date is the next cba's file date, if they have one. otherwise, keep the original end date
...
gen group_id_firm = identificad + "_" +union_id + "_"+ string(active_year) // we are going to collapse the dataset by estab id within each year where the estab has an active cba

// define minimum and maximum file dates per group



bys group_id_firm: egen max_file_date = max(file_date_stata)
format max_file_date %td



bys group_id_firm: egen min_file_date = min(file_date_stata)
format min_file_date %td


// Define treatment status at the group_id level 
...
collapse ///
    (firstnm) identificad_8 identificad union_id active_year act mode_base_month is_main_union n_negs_union_year min_file_date max_file_date /// Variables to keep as is. choose the first that is non missing
...
gen avg_file_date = start_date_stata+negotiation_days // regenerate file date according to avg neg months.

format avg_file_date %td 
...
keep identificad_8 identificad union_id is_main_union text_tokens end_date_stata start_date_stata n_negs_union_year min_file_date avg_file_date max_file_date active_year negotiation_months mode_base_month ultra treat_ultra numb_clauses_original numb_clauses cl_*

gen start_year = year(start_date_stata)
gen end_year = year(end_date_stata)

gen b_unit = identificad+ "_"+union_id


* Save Bargaining-unit level dataset:
save "$cba_dir/collapsed_cba_bunit_updated.dta", replace


* Save Establishment-main union level dataset:
keep if is_main_union==1

save "$cba_dir/collapsed_cba_firm_updated.dta", replace

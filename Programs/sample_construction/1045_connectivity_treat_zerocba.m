...
% Data directory. Parameterised so the chain can run against a scratch copy
% without touching production. Falls back to the production path when the
% environment variable is unset, so existing behaviour is unchanged.
...
%% Load Zero CBA Treat Sample Dataset
zerocbatreat = readtable(fullfile(auxdir, 'zero_cba_treat.csv'));
zerocbatreat.identificad = string(zerocbatreat.identificad);

%% Initialize cell to store connectivity measures
conntables = {};

%% Loop: compute connectivity matrix and sum flows to zero_cba_treat sample for every two consecutive years:
for y = start_year:(end_year-1)
    next_y = (y+1);

    % Read employers across two consecutive years datasets:
    filename = fullfile(auxdir, sprintf('employers_%d_%d.csv', y, next_y));
    ...
    % Construct adjacency matrix for establishments, zeros main diagonal
    M_estabs = sparse(estab_y_idx, estab_next_idx, 1, n_estabs, n_estabs);
    M_estabs(1:n_estabs+1:end) = 0;

    % Mark which estabs are part of zero_cba_treat sample:
    [~, zero_idx] = ismember(unique_estabs, zerocbatreat.identificad);
    is_zero = zeros(n_estabs,1);
    is_zero(zero_idx > 0) = zerocbatreat.zero_cba_treat(zero_idx(zero_idx > 0));
    ...
    outToZero = full(sum(M_estabs(:, is_zero == 1), 2));
    inFromZero = full(sum(M_estabs(is_zero == 1, :), 1))';
    ZeroFlows = outToZero + inFromZero;
    ...
    norm_outToZero = outToZero ./ avg_emp;
    norm_inFromZero = inFromZero ./ avg_emp;
    norm_ZeroFlows = ZeroFlows ./ avg_emp;

    % Normalize flows as a ratio of total flows
    norm_ZeroFlows_pf = ZeroFlows ./ totalFlows;
    ...
    T_pair = table(unique_estabs, ...
                ...
                outToZero, inFromZero, ZeroFlows, ...
                norm_outToZero, norm_inFromZero, norm_ZeroFlows, ...
                norm_ZeroFlows_pf, ...
                'VariableNames', { ...
                    'identificad1', ...
                    'total_out','total_in','total_flows', ...
                    'total_out_pw','total_in_pw','total_flows_pw', ...
                    'out_zero','in_zero','zero_flows', ...
                    'out_zero_pw','in_zero_pw','zero_flows_pw', 'zero_flows_pf' ...
                });
    ...
    outtlVarName = sprintf('outzero_%d_%d',y,next_y);
    inftlVarName = sprintf('inzero_%d_%d', y, next_y);
    totlVarName  = sprintf('totalzero_%d_%d',y,next_y);
    ...
    T_pair.Properties.VariableNames{'out_zero'} = outtlVarName;
    T_pair.Properties.VariableNames{'in_zero'} = inftlVarName;
    T_pair.Properties.VariableNames{'zero_flows'} = totlVarName;
    ...
    outtlVarName_pw = sprintf('outzero_pw_%d_%d', y, next_y);
    inftlVarName_pw = sprintf('inzero_pw_%d_%d', y, next_y);
    totlVarName_pw  = sprintf('totalzero_pw_%d_%d', y, next_y);
    totlVarName_pf  = sprintf('totalzero_pf_%d_%d', y, next_y);
    ...
    T_pair.Properties.VariableNames{'out_zero_pw'}    = outtlVarName_pw;
    T_pair.Properties.VariableNames{'in_zero_pw'}     = inftlVarName_pw;
    T_pair.Properties.VariableNames{'zero_flows_pw'}  = totlVarName_pw;
    T_pair.Properties.VariableNames{'zero_flows_pf'}  = totlVarName_pf;
...
outputFile = fullfile(auxdir, 'connectivity_zero_2007_2011.csv');

...
lagos = readtable(fullfile(auxdir, 'lagos_sample.csv'));
% convert employer id to string:
% 1040 exports lagos_sample.csv with the column named `identificad` (it renames
% identificad1 -> identificad before the export). 1052-1055 all read `.identificad`;
% only this script still referenced `.identificad1`, so it failed with
% "Unrecognized table variable name". The OUTPUT column stays identificad1,
% which is what 1050 imports. See Docs/pipeline/TIER_A_DEFECTS.md A8.
lagos.identificad = string(lagos.identificad);
...
%employers = readtable(fullfile(auxdir, 'employers_2007_2008.csv'));
...
for y = start_year:(end_year-1)
        ...
        [~, lagos_idx] = ismember(unique_estabs, lagos.identificad);
        is_lsample = zeros(n_estabs,1);
        % Column is lagos_sample_avg, which is what 1040 collapses and exports.
        is_lsample(lagos_idx > 0 ) = lagos.lagos_sample_avg(lagos_idx(lagos_idx>0));
...
outputFile = fullfile(auxdir, 'connectivity_2007_2011.csv');  

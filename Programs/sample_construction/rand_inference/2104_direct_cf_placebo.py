...
WHAT IS HELD FIXED (this is the core rule of the design):
  ...
  * the outcome, fixed effects, clustering, weights and event-time structure --
    identical to 3012_pct_tfpw.do (lines 320-384);
...
exercise (2093_permutation_engine.py, 2096_placebo_diag.py): the treated label is
...
Stratification schemes are copied verbatim from 2096_placebo_diag.py, INCLUDING the
correct 2-digit CNAE division (04 uses `industry1 // 100`, which is wrong -- see
1030_merge_cba_rais.do:169-172, industry1 carries an artificial leading "1").
...
not informative. 2106_direct_cf_placebo_report.py reports those diagnostics.
...
# ── permutation universe + CEM strata (verbatim from 2096_placebo_diag.py) ─────

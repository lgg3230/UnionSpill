...
Validation checks for the reconstructed whole-RAIS Mincer residualization.
  ...
  S4  per-cell residual mean ~ 0. This is the check that catches the
      ...
      ~1e-14) described in 2083_residualize_fullrais.py.
...
Usage
-----
    python 2084_safeguards_fullrais.py ref \
        --new mincer_residuals_firm_year_ten_fullrais_y2011.csv \
        --ref mincer_residuals_firm_year_ten_fullrais.csv \
        --tol 1e-10

    python 2084_safeguards_fullrais.py s4 --years 2011 --mode agetenure --min-obs 10
"""
...
def check_s4(args):
    # "2083_residualize_fullrais" is not a valid module name, so load it by path.
    # run_name is not "__main__", so the module's main() does not fire.
    import runpy
    mod = runpy.run_path(str(Path(__file__).resolve().parent / "2083_residualize_fullrais.py"),

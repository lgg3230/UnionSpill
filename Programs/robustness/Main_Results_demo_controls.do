...
*            (2) + Gender   (male_prop)
*            (3) + Race     (white_prop)
*            (4) + Education (prop_hs_plus)
*            (5) + Age      (avg_age)
*            (6) + Tenure   (avg_tenure)
* Output:   Tables/robustness/results_spill_demo_controls.csv
********************************************************************************

capture log close
local d = subinstr("`c(current_date)'"," ","_",.)
local t = subinstr("`c(current_time)'",":","",.)
log using "$logs/robustness/Main_Results_demo_controls_`d'_`t'.log", replace text
...
capture erase "$tables/robustness/results_spill_demo_controls.csv"
tempname fh
file open `fh' using "$tables/robustness/results_spill_demo_controls.csv", write replace
file write `fh' "outcome;col;row_type;value" _n
file close `fh'

local csv     "$tables/robustness/results_spill_demo_controls.csv"
...
* Demographic quartile × year terms (added cumulatively)
local demo_gender    "ib0.male_prop_pre4#i.year"
local demo_race      "ib0.white_prop_pre4#i.year"
local demo_educ      "ib0.prop_hs_plus_pre4#i.year"
local demo_age       "ib0.avg_age_pre4#i.year"
local demo_tenure    "ib0.avg_tenure_pre4#i.year"

* ── Helper macro: write one col's results ────────────────────────────────────

* (Stata doesn't support sub-programs easily, so repeat pattern per column)
...
di as result "Col 3 (+ gender + race)"
...
di as result "Col 4 (+ gender + race + education)"
...
di as result "Col 5 (+ gender + race + education + age)"
...
di as result "Col 6 (all demographics: + tenure)"
...
di _newline(1)
di as result "All demographic controls regressions complete."
di as result "Finished: `c(current_date)' `c(current_time)'"

log close

shell ~/.conda/envs/venv_python312/bin/python "$programs/robustness/generate_demo_controls_latex.py"
di as result "LaTeX table written to Tables/robustness/demo_controls_table.tex"

shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "Stata done" "Main_Results_demo_controls.do complete"

...
foreach outcome in lr_remdezr_w lr_remdezr_h_w l_firm_emp lr_remdezr_stayers lr_remdezr_switchers turnover {
...
foreach g of local glist{
	foreach wage in lr_remdezr_w lr_remdezr_switchers lr_remdezr_stayers lr_remdezr_h_w l_firm_emp numb_clauses lr_remdezr_w_p10 lr_remdezr_w_p20 lr_remdezr_w_p25 lr_remdezr_w_p50 lr_remdezr_w_p75 lr_remdezr_w_p80 lr_remdezr_w_p90 lr_remdezr_h_w_p10 lr_remdezr_h_w_p20 lr_remdezr_h_w_p25 lr_remdezr_h_w_p50 lr_remdezr_h_w_p75 lr_remdezr_h_w_p80 lr_remdezr_h_w_p90 lr_remdezr_w_p90p10 lr_remdezr_w_p80p20 lr_remdezr_w_p75p25 lr_remdezr_w_p90p50 lr_remdezr_w_p80p50 lr_remdezr_w_p75p50 lr_remdezr_h_w_p90p10 lr_remdezr_h_w_p80p20 lr_remdezr_h_w_p75p25 lr_remdezr_h_w_p90p50 lr_remdezr_h_w_p80p50 lr_remdezr_h_w_p75p50 turnover {
...
foreach spec of global specs {
    ...
    * Extract group number if applicable
    ...
    else if regexm("`spec'", "^TFPE_BIN_([0-9]+)$") {
        local g = regexs(1)
...
// di as text "  • Results CSVs: $tables/results_[direct|spill]_[spec].csv"
...
// among untreated firms, how many of them are covered by a union that also covers treated firms?
...
// generate geographical sharing of treatment: we needc to scale these measures as well to compare coefficients


bys microregion (year): egen geo_exp = mean(treat_ultra)	// create share of treated for each microregion
label var geo_exp "share of treated among sample in a given microregion"

bys municipio (year): egen geo_mun_exp = mean(treat_ultra) // create share of treated for each municipality
label var geo_mun_exp "share of treated among sample in a given municipality"

bys year: egen mun_spill_count = nvals(municipio) if treat_ultra==0 & in_balanced_panel==1 & lagos_sample_avg == 1 // counts the number of municipalities in the spillover sample
...
label var mic_spill_count "counts number of microregion in spillover sample"
...
// generate microregion-industry indicator:
...
label var mic_ind_exp "what proportion of in-sample firms are treated within each micro-industry cell "

gen mic_ind_treat = cond(mic_ind_exp>0,1,0)
label var mic_ind_treat " indicates whether a firm's micro-industry cell also covers at least one treated firm"
...
bys industry1 (year): egen ind_exp_o = mean(treat_ultra)	// create share of treated for each industry

// destring identificad, generate(cnpj_num) force
// isid identificad // yes
// isid cnpj_num // yes
//
// keep if year ==2009
...
// isid cnpj_num // yes
//
// collapse (count) cnpj_num (firstnm) union_emp_exp treat_union_exp_all, by(mode_union)

// how well do other measures of distance predict flows?

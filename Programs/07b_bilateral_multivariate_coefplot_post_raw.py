...
AUTHOR: LUIS GOMES
PROGRAM: COEFFICIENT PLOTS - POST-TREATMENT BILATERAL CONNECTIVITY (RAW DV)
INPUT: bilateral_multivariate_coefficients_post_raw.csv
...
COEF_FILE = RAIS_AUX / "bilateral_multivariate_coefficients_post_raw.csv"
...
VAR_LABELS = {
    ...
    "bilateral_conn_pre": "Pre-treatment Bilateral Conn.",
...
def create_coefplot(df, variables, output_path, color="#1f4e79", marker="o", xlabel="Coefficient"):
...
def main():
    ...
    print("=" * 60)
    print("BILATERAL CONNECTIVITY COEFFICIENT PLOTS (POST, RAW DV)")
    print("=" * 60)
    ...
    # Figure 1: Continuous measures (including pre-treatment conn)
    print("\n--- Creating continuous measures plot (raw DV) ---")
    df_continuous = df[
        (df["var_type"] == "continuous") | (df["var_type"] == "pre_connectivity")
    ].copy()

    output_continuous = GRAPHS / "coefplot_bilateral_continuous_post_raw.pdf"
    create_coefplot(
        df_continuous,
        CONTINUOUS_VARS,
        output_continuous,
    ...
    output_dummy = GRAPHS / "coefplot_bilateral_dummies_post_raw.pdf"
    create_coefplot(
        df_dummy,
        DUMMY_VARS,
        output_dummy,
    ...
    print("Pre-treatment connectivity coefficient shows persistence of bilateral flows.")

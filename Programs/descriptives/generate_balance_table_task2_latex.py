...
def main():
    ...
    L.append(
        ...
        r"main-specification controls (two-digit industry, microregion, and "
        r"negotiation-month indicators, plus quartile-bin controls for "
        r"pre-treatment establishment size, per-worker pairwise worker flows "
        r"(2007--2011), and pre-treatment log wages and log employment measured "
        r"over 2009--2010 to avoid conditioning on the same-period outcome). "
        ...
        r"applied indicate the covariates absorb the relevant selection. For the "
        r"worker-flows row, its own pre-treatment flows control is omitted from the "
        r"controlled columns so the variable is not mechanically adjusted for a "
        r"discretized version of itself. Tenure and "

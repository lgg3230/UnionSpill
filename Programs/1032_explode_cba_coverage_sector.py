...
cba_sector = pd.read_stata(os.path.join(main, 'UnionSpill/Data/CBA/cba_coverage_clean_sector.dta'))
...
cba_sector_exp = cba_sector.explode('codigo_municipio').reset_index(drop=True)

# Clean text for all string columns
for col in cba_sector_exp.select_dtypes(include=['object']).columns:
    cba_sector_exp[col] = cba_sector_exp[col].apply(clean_text)

cba_sector_exp.to_stata(os.path.join(main,'UnionSpill/Data/CBA/cba_sector_exploded.dta'), write_index=False)

...
local s_direct "(lagos_sample_avg==1 & treat_ultra==0 & avg_ftreat_pf_n<0.01 | lagos_sample_avg==1 & treat_ultra==1)"
...
// local cont_controls "firm_emp_pre"


* Categorical (rotate as time-varying FE i.var#time)
// local cat_controls  "natjuridica1 mode_union_1"
local cat_controls  ""
...
local OUT_A  "l_firm_emp lr_remdezr lr_remmedr"   // Family A
// local CONN_A "totaltreat_pf_n totaltreat_pw_n avg_ftreat_pf_n"
local CONN_A "totaltreat_pw_n "
...
* Column labels (12 per outcome; for Family A repeat 3×)
...
* For Family B (clauses only) we'll reuse mcols but only once
...
foreach pw in `pre_windows' {
    ...
    * === NEW: window filters for this loop ===
    local WIN_POST "inrange(year, `pstart', 2016)"   // pooled DiD window starts at pstart
    local WIN_PRE  "inrange(year, `pstart', 2011)"     // pre-trends window ends at 2011

    foreach coding in `codings' {
        foreach C in `cont_controls' `cat_controls' {

            * ---- Build additional control terms (timing-specific) ----
            local ADD_POST ""   // used with treat_year
            local ADD_PRE  ""   // used with placebo_year

            * FIX: use mode_union_1 here
            if inlist("`C'","natjuridica1","mode_union") {
                local ADD_POST "i.`C'#treat_year"
                local ADD_PRE  "i.`C'#placebo_year"
            }
            else {
                local V ""
                ...
                if "`coding'"=="quartile" & "`C'"!="totalflows_n" {
                    ...
                    local ADD_POST "i.q4_`V'#treat_year"
                    local ADD_PRE  "i.q4_`V'#placebo_year"
                }
                if "`coding'"=="quintile" & "`C'"!="totalflows_n" {
                    ...
                    local ADD_POST "i.q5_`V'#treat_year"
                    local ADD_PRE  "i.q5_`V'#placebo_year"
                }
                if inlist("`coding'","quartile","quintile") & "`C'"=="totalflows_n" {
                    local ADD_POST "`V'#treat_year"
                    local ADD_PRE  "`V'#placebo_year"
                }
            }
...
local ++estnum
                ...
                * ---- Export MEGA table (3 outcomes × 12 columns each = 36 cols) ----
				
				* ---------- after the reghdfe calls, just before esttab ----------

// * NEW/CHANGED: with only 2 models per outcome, define matching labels
// local mcols_direct "Direct-Post Direct-Pre"
// local mlabels_A_short "`mcols_direct' `mcols_direct' `mcols_direct'"

                esttab using "results/tables/MEGA_A_`conn'_`C'_`coding'_p`pw'_Oct25.csv", replace ///
                    keep( ///
                        ...
                        1.treat_year#c.`CONN_IN_pf'    1.placebo_year#c.`CONN_IN_pf' ) se star(* 0.10 ** 0.05 *** 0.01) b(3) se(3) 
                   
//                     mlabels(`"`mlabels_A'"') ///
// //                     mgroups("l_firm_emp" "lr_remdezr" "lr_remmedr"), pattern(12 12 12) ///

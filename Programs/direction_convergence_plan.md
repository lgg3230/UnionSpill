...
- `i` indexes the firm / establishment / CBA unit;
...
Merge the treated-firm `cba_period == 2` vectors onto the pairwise untreated-treated connectivity dataset by treated firm ID.

Then normalize weights within untreated firm ID:

`w_ij = flow_ij / sum_j flow_ij`

Use the same flow/connectivity object as the main spillover analysis.

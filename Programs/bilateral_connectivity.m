...
sample_file = '/kellogg/proj/lgg3230/UnionSpill/Data/RAIS_aux/sample_establishments.csv'; % Path to sample establishments CSV
sample_estabs = readtable(sample_file, 'VariableNamingRule', 'preserve');       % Read sample establishments from CSV, preserving variable names
sample_ids = string(sample_estabs.identificad);                                 % Convert establishment IDs to string array
n_sample = length(sample_ids);                                                  % Count number of sample establishments
fprintf('Loaded %d sample establishments.\n', n_sample);                        % Print number of sample establishments
...
for y = start_year:(end_year-1)                                                 % Loop through years 2007 to 2010 (last year before end_year)
    ...
    employers = readtable(filename, 'VariableNamingRule', 'preserve');          % Read employers data from CSV into a table, preserving variable names
    ...
    in_sample_y = ismember(estab_y, sample_ids);                                % Check if current year establishment is in sample
    in_sample_next = ismember(estab_next, sample_ids);                          % Check if next year establishment is in sample
    valid_sample = in_sample_y & in_sample_next;                                % Both must be in sample

    fprintf('  Rows before sample filter: %d\n', length(estab_y));              % Print number of rows before filtering
    fprintf('  Rows after sample filter: %d\n', sum(valid_sample));             % Print number of rows after filtering

    % Apply filter
    estab_y = estab_y(valid_sample);                                            % Keep only current year establishments in sample
    estab_next = estab_next(valid_sample);                                      % Keep only next year establishments in sample
    employers = employers(valid_sample, :);                                     % Filter employers table to sample rows
    ...
    estab_i_list = strings(n_pairs, 1);                                         % Initialize string array for origin establishment IDs
    estab_j_list = strings(n_pairs, 1);                                         % Initialize string array for destination establishment IDs
...
pair_first = strings(height(final_bilateral), 1);                               % Initialize string array for first (smaller) establishment in pair
pair_second = strings(height(final_bilateral), 1);                              % Initialize string array for second (larger) establishment in pair
...
pair_key = strcat(pair_first, '_', pair_second);                                % Create unique string key by concatenating pair IDs
...
out_first = strings(n_unique_pairs, 1);                                         % Initialize string array for first establishment IDs in output
out_second = strings(n_unique_pairs, 1);                                        % Initialize string array for second establishment IDs in output
...
for p = 1:n_unique_pairs                                                        % Loop through each unique pair
    if mod(p, 100000) == 0                                                      % Print progress every 100,000 pairs
        fprintf('  Processed %d / %d pairs...\n', p, n_unique_pairs);           % Print progress update
    end
    ...
    for row = 1:height(subset)                                                  % Loop through rows in subset for this pair
        orig_i = subset.estab_i(row);                                           % Get original establishment i from this row
        first_estab = subset.pair_first(row);                                   % Get canonical first establishment

...
# Fixture, not pipeline output: no script in Programs/ produces these. The old
# RAIS_AUX path pointed at files that do not exist, so this script could not run.
FIXTURES = BASE_DIR / "Docs" / "fixtures" / "figure_A2"
GRAVITY_UNIV_FILE = FIXTURES / "coef_connectivity_univ.csv"
GRAVITY_MULTI_FILE = FIXTURES / "coef_connectivity_multi.csv"

# 07d regression: predicting late-pre connectivity with early connectivity + proximity (split files)
PRETREAT_UNIV_FILE = FIXTURES / "coef_late_connectivity_univ.csv"
PRETREAT_MULTI_FILE = FIXTURES / "coef_late_connectivity_multi.csv"

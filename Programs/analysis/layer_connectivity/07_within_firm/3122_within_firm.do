********************************************************************************
* archive/Programs/layer_connectivity/07_within_firm/01b_within_firm_estimates.do
* V2 within-firm specification ported from Programs/within_firm_final/R/archive/Programs/within_firm_final/R/04_engine.R.
* Produces the estimate CSVs for Tables A6, A7, A8 (edu2, gender, ten2):
*     $tables/a7.csv        (schema: partition,col,outcome,b,se,bpre,sepre,n,firms,gxf)
*     $tables/a8.csv        (partition,col,p90,outcome,b1,se1,b2,se2,bp1,sp1,bp2,sp2,peq,n,firms)
*     $tables/a6_group.csv  (partition,group,avg_emp,avg_wage,avg_flows,conn)
*     $tables/a6_partition.csv (partition,pct_both,v_tot,v_wi,v_bw,wi_pct,bw_pct)
*
...
* Mirrors the bin construction in 3012_pct_tfpw.do so that the
...
program define balance_post
    * args: posthandle partition col outcome rawcondition samplevar
    ...
    foreach v in bal_pre_group_emp bal_pre_group_wage bal_group_conn bal_pre_firm_size bal_pre_group_share {
        ...
        post `handle' ("monthly") ("`partition'") ("`col'") ("`outcome'") ("`v'") ("kept") (r(N)) (r(mean))
        quietly summarize `v' if (`cond') & `samplevar'==0
        post `handle' ("monthly") ("`partition'") ("`col'") ("`outcome'") ("`v'") ("dropped") (r(N)) (r(mean))
    }
    ...
    foreach status in 1 0 {
        ...
        restore
        post `handle' ("monthly") ("`partition'") ("`col'") ("`outcome'") ("microregion_distinct") ("`sname'") (`total') (`ndist')
        post `handle' ("monthly") ("`partition'") ("`col'") ("`outcome'") ("microregion_hhi") ("`sname'") (`total') (`hhi')
    }
end
...
keep identificad year lr_remdezr_w l_firm_emp treat_ultra in_balanced_panel ///
...
mkprebinh lr_remdezr_w_pre4 lr_remdezr_w identificad "" mean ""
...
use `FIRM', clear
foreach y in lr_remdezr_w l_firm_emp {
    local oc = cond("`y'"=="l_firm_emp","emp","wage")
    if "`y'"=="l_firm_emp" local bins "i.l_firm_emp_pre4#i.year i.totalflows_pw_pre4#i.year"
    else                   local bins "i.lr_remdezr_w_pre4#i.year i.l_firm_emp_pre4#i.year i.totalflows_pw_pre4#i.year"
...
foreach p in `partitions' {
    ...
    * group-level pre-treatment quartile bins (by firm x layer)
    mkprebin lr_remdezr_layer_pre4      lr_remdezr_layer     identificad layer_id mean
    ...
    mkprebin layer_totalflows_pw_pre4   totalflows_layer_pw_n identificad layer_id raw
	    ...
	    * firm-level pre-treatment bins for the v2 overall column
	    merge m:1 identificad year using `FIRM', ///
	        keepusing(lr_remdezr_w_pre4 l_firm_emp_pre4 totalflows_pw_pre4) ///
	        keep(master match) nogen
	    ...
	    * pre-treatment balance variables for v2 singleton-dropped rows
	    ...
	    gen double __bal_wage_level = exp(lr_remdezr_layer)
    ...
    * ------------------------------------------------------------------
    preserve
        ...
        gen double wlev = exp(lr_remdezr_layer)
    ...
    * ------------------------------------------------------------------
	    ...
	    foreach kind in within overall {
	        foreach y in lr_remdezr_layer l_layer_emp {
	            ...
	            else                    local bins "i.layer_id_num#i.lr_remdezr_layer_pre4#i.year i.layer_id_num#i.l_layer_emp_pre4#i.year i.layer_id_num#i.layer_totalflows_pw_pre4#i.year"
	            ...
	            if "`kind'"=="within" local fes "i.firm_layer_id `bins' i.firm_id#i.year `groupfes'"
	                ...
	                else                    local firmbins "i.lr_remdezr_w_pre4#i.year i.l_firm_emp_pre4#i.year i.totalflows_pw_pre4#i.year"
    ...
    * ------------------------------------------------------------------
    ...
    foreach gl in `g1' `g2' {
        foreach y in lr_remdezr_layer l_layer_emp {
            ...
            else                    local bins "i.lr_remdezr_layer_pre4#i.year i.l_layer_emp_pre4#i.year i.layer_totalflows_pw_pre4#i.year"
...
use `A7dta',  clear
export delimited using "$tables/a7`table_suffix'.csv", replace
use `A8dta',  clear
export delimited using "$tables/a8`table_suffix'.csv", replace
use `A6Gdta', clear
export delimited using "$tables/a6_group`table_suffix'.csv", replace
use `A6Pdta', clear
export delimited using "$tables/a6_partition`table_suffix'.csv", replace
use `BALdta', clear
export delimited using "$tables/singleton_balance`table_suffix'.csv", replace

di as result "Wrote a7`table_suffix'.csv, a8`table_suffix'.csv, a6_group`table_suffix'.csv, a6_partition`table_suffix'.csv, singleton_balance`table_suffix'.csv to $tables"

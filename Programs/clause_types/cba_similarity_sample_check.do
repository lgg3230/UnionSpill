...
log using "$logs/clause_types/cba_similarity_sample_check_`d'_`t'.log", replace text
...
drop m_bilat
foreach v in cosine bray_curtis total_variation jaccard {
	rename `v' `v'_b
}
...
foreach v in cosine_b bray_curtis_b total_variation_b jaccard_b ///
             cosine bray_curtis total_variation jaccard {
	cap drop `v'_pre_o
	cap drop `v'_pre
	quietly {
...
local bilat_list "cosine_b bray_curtis_b total_variation_b jaccard_b"
local avg_list   "cosine   bray_curtis   total_variation   jaccard"

********************************************************************************
* SECTION 6: INITIALIZE OUTPUT CSV
********************************************************************************

local csv "$tables/clause_types/results_sample_check_cba_similarity.csv"
capture erase "`csv'"
tempname fh
...
shell ~/.conda/envs/venv_python312/bin/python "$programs/clause_types/generate_sample_check_latex.py"

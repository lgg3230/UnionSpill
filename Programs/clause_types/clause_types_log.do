...
*
* Transform: l1p_<count> = ln(1 + <count>)   [log1p, keeps zeros]
*   Chosen over asinh: numb_clauses has near-zero mass at 0 (4/122k) and large
*   support (0-160), so log1p and asinh are materially identical here while
*   log1p reads cleanly as an approximate proportional change.
*
* Outcomes (log1p of the raw count):
...
*
* Output:
*   Tables/clause_types/results_direct_panel[A/B/C]_clause_counts_log_tfpw_07_11.csv
*   Tables/clause_types/results_spill_clause_counts_log_tfpw_07_11.csv
*   Tables/clause_types/clause_count_log_direct_effects.tex
*   Tables/clause_types/clause_count_log_spillover_effects.tex
*   Graphs/clause_types/es_l1p_numb_clauses_direct[A/B/C]_<date>.pdf
*   Graphs/clause_types/es_l1p_numb_clauses_spill_<date>.pdf
********************************************************************************
...
log using "$logs/clause_types/clause_types_log_`d'_`t'.log", replace text
...
********************************************************************************
* SECTION 2: CLAUSE COUNT VARIABLES (raw counts, then log1p transform)
********************************************************************************
...
* ── log1p transform of the four count outcomes (keeps zeros) ────────────────
foreach c in numb_clauses wage_clauses emp_clauses other_clauses {
	cap drop l1p_`c'
	gen double l1p_`c' = ln(1 + `c') if !missing(`c')
	label var l1p_`c' "Log(1 + `c')"
}
...
* Pre-treatment means + RAW quartile bins for the four count outcomes.
* The bins are computed on the RAW counts and reused as controls for the
* log1p outcomes (only the dependent variable changes).
...
local spec "clause_counts_log_tfpw_07_11"
local conn "totaltreat_pw_norm"

* Outcomes are the log1p transforms; the matching RAW count name (for pre4 bins)
* is recovered by stripping the "l1p_" prefix inside the loops.
local count_outcomes "l1p_numb_clauses l1p_wage_clauses l1p_emp_clauses l1p_other_clauses"
...
foreach panel in A B C {
	capture erase "$tables/clause_types/results_direct_panel`panel'_clause_counts_log_tfpw_07_11.csv"
	tempname fh
	file open `fh' using "$tables/clause_types/results_direct_panel`panel'_clause_counts_log_tfpw_07_11.csv", write replace
	file write `fh' "spec;section;outcome;row_type;value" _n
	file close `fh'
}

capture erase "$tables/clause_types/results_spill_clause_counts_log_tfpw_07_11.csv"
tempname fh
file open `fh' using "$tables/clause_types/results_spill_clause_counts_log_tfpw_07_11.csv", write replace
file write `fh' "spec;section;outcome;row_type;value" _n
file close `fh'
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "DIRECT EFFECTS (log1p counts) — PANELS A, B, C"
di as result "-----------------------------------------------------------------------"
...
foreach panel in A B C {
	...
	local csv_out "$tables/clause_types/results_direct_panel`panel'_clause_counts_log_tfpw_07_11.csv"

	foreach outcome of local count_outcomes {

		* raw count name (for the pre4 bin control) = outcome minus "l1p_"
		local rawc = substr("`outcome'", 5, .)

		di as text "  Estimating: `outcome' (Panel `panel')"

		local absorb_cba "`base_fe_cba' ib0.`rawc'_pre4#i.cba_period ib0.l_firm_emp_pre4#i.cba_period `extra_cba'"
		...
		* ── Event-study plot — l1p_numb_clauses only (Main_Results parity) ──
		if "`outcome'" == "l1p_numb_clauses" {
			...
			coefplot _es_d_tmp, ///
				...
				ytitle("Dynamic DiD coefficients (log1p clauses)", size(small)) ///
				note("P-value for placebo pre-trend = `pre_pval_s'") ///
				graphregion(color(white)) bgcolor(white) ///
				ci(95) ciopts(recast(rcap) color(blue)) mcolor(blue) ///
				text(0.05 4 "`post_coef_s' (`post_se_s')", color(blue) size(small))

			cap graph export "$graphs/clause_types/es_l1p_numb_clauses_direct`panel'_`d'.pdf", as(pdf) replace
...
di _newline(2)
di as result "-----------------------------------------------------------------------"
di as result "SPILLOVER EFFECTS (log1p counts)"
di as result "-----------------------------------------------------------------------"

local csv_spill "$tables/clause_types/results_spill_clause_counts_log_tfpw_07_11.csv"
...
foreach outcome of local count_outcomes {
	...
	* ── Event-study plot — l1p_numb_clauses spillover only (Main_Results parity) ──
	if "`outcome'" == "l1p_numb_clauses" {
		...
		coefplot _es_sp_tmp, ///
			...
			graphregion(color(white)) bgcolor(white) ///
			ci(95) ciopts(recast(rcap) color(blue)) mcolor(blue) ///
			text(0.02 4 "`post_coef_s' (`post_se_s')", color(blue) size(small))

		cap graph export "$graphs/clause_types/es_l1p_numb_clauses_spill_`d'.pdf", as(pdf) replace
		estimates drop _es_sp_tmp
	}
}

di as result "All log1p clause-count regressions complete."

********************************************************************************
* SECTION 8: AUTO-GENERATE LATEX TABLES
********************************************************************************

log close
di as result "Finished: `c(current_date)' `c(current_time)'"

shell ~/.conda/envs/venv_python312/bin/python "$programs/clause_types/generate_clause_count_log_latex.py"
di as result "LaTeX tables written to Tables/clause_types/clause_count_log_*.tex"

shell source "$programs/notify.sh" && notify "clause_types_log done" "log1p clause-count direct+spillover regressions, tables, and graphs complete"

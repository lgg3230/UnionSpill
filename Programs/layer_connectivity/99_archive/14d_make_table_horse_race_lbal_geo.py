"""
Assemble horse race table from 13d_horse_race_edu2_lbal_geo.do output.
Layer-balanced panel + microregion×year, mode_base_month×year, industry×year FEs.
...
Output:
  Tables/layer_connectivity/table_horse_race_edu2_lbal_geo.tex / .csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/14d_make_table_horse_race_lbal_geo.py
"""
...
def build_latex(data: dict, caption: str, label: str) -> str:
    ...
    for row_label, marks in [
        ...
        ("Microregion $\\times$ Year", checkmarks_all),
        ("Base month $\\times$ Year",  checkmarks_all),
        ("Industry $\\times$ Year",    checkmarks_all),
    ...
    lines.append(
        ...
        r"worker layer and absorb firm$\times$layer FE, year FE, and microregion$\times$year, "
        r"base-month$\times$year, and industry$\times$year fixed effects. "
        r"Columns (3) and (6) are firm-level regressions that collapse to firm$\times$year "
        r"and absorb the same set of fixed effects. "
...
if not CSV_IN.exists():
    raise SystemExit(f"ERROR: results file not found — run 13d_horse_race_edu2_lbal_geo.do first:\n  {CSV_IN}")
...
caption = (
    r"Horse race: $c_{\text{no\_hs}}$ vs.\ $c_{\text{has\_hs}}$ simultaneously "
    r"(layer-balanced panel, geo/sector FEs)"
)

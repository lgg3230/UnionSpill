...
global table_suffix "_hlogic_ml"
global SIZE_DEF "meanlog"

cap mkdir "$tables"
cap mkdir "$logs"

capture log close
log using "$logs/within_firm_hlogic_ml_hw_v2.log", replace text
...
do "$programs/01c_within_firm_estimates_hw.do"
di as result "Finished: `c(current_date)' `c(current_time)'"
shell source /gpfs/kellogg/proj/lgg3230/UnionSpill/Programs/notify.sh && notify "within_firm_hw_v2" "Hourly v2 within-firm Stata job finished"
log close

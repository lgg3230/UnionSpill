...
Reads CSVs produced by 01a_layer_spillover.do and outputs:
  Tables/layer_connectivity/table_layer_specs_firm.tex
  Tables/layer_connectivity/table_layer_specs_firm.csv
  Tables/layer_connectivity/table_layer_specs_firm_edu.tex
  Tables/layer_connectivity/table_layer_specs_firm_edu.csv
  Tables/layer_connectivity/table_layer_specs_firm_demog.tex
  Tables/layer_connectivity/table_layer_specs_firm_demog.csv

Specs included:
  (1) Within-firm FE     — firm×year FE, layer connectivity
                           results_spill_layer_{edu2,gender,race}_firm_layer_spill.csv
  (2) Cross-firm FE      — micro×year + industry×year + mode×year FE
                           results_spill_layer_cross_{edu2,gender,race}_firm_layer_spill.csv
  (3) Firm-level restr.  — firm-level outcomes, standard FE, restricted sample
                           results_spill_firmrestr_{edu2,gender,race}_firm_layer_spill.csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/02a_make_table_spillover.py
"""
...
def build_latex(layers_subset: list, caption: str, label: str) -> str:
    ...
    lines.append(
        ...
        r"The sample covers all firms in the Lagos sample. All layers of qualifying firms are kept. "
...
LABELS = {
    "edu":   "tab:layer_specs_firm_edu",
    "demog": "tab:layer_specs_firm_demog",
}

for group_key, (layers_subset, _) in LAYER_GROUPS.items():
    tex_out = TEX_TABLES / f"table_layer_specs_firm_{group_key}.tex"
    csv_out = TABLES / f"table_layer_specs_firm_{group_key}.csv"
...
# Combined table
tex_out_all = TEX_TABLES / "table_layer_specs_firm.tex"
csv_out_all = TABLES / "table_layer_specs_firm.csv"
tex_all = build_latex(LAYERS, "Layer spillover effects --- specification comparison", "tab:layer_specs_firm")

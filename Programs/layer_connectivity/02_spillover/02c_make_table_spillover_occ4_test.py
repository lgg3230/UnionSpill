...
SPECS = [
    {
        ...
        "file_tpl": "results_spill_layer_{layer}_firm_layer_spill_nolayeryear.csv",
        "outcomes": ["lr_remdezr_layer", "l_layer_emp"],
    },
    {
        "label":    "(2) Cross-firm FE",
        "sublabel": r"micro$\times$yr + ind$\times$yr + mode$\times$yr",
        "file_tpl": "results_spill_layer_cross_{layer}_firm_layer_spill_nolayeryear.csv",
        "outcomes": ["lr_remdezr_layer", "l_layer_emp"],
    },
    {
        "label":    "(3) Firm-level (restricted)",
        "sublabel": "firm FE",
        "file_tpl": "results_spill_firmrestr_{layer}_firm_layer_spill_nolayeryear.csv",
...
def build_latex() -> str:
    ...
    lines.append(
        r"\caption{Layer spillover effects --- occupation layers, no layer$\times$year FE (test)}"
    )
    lines.append(r"\label{tab:layer_specs_occ4_nolayeryear}")
...
tex_out = TEX_TABLES / "table_layer_specs_occ4_nolayeryear.tex"
csv_out = TABLES / "table_layer_specs_occ4_nolayeryear.csv"

"""
Assemble disentangling tables using cross-firm FE specifications.

Identical to 12_make_table_disentangle.py but reads:
  section = "layer_cross_firm"  (instead of "layer_firm_year")
  section = "firm_cross_firm"   (instead of "firm_firm_year")

These sections are already in the CSV output of 11_disentangling_layers.do.

Output:
  Tables/layer_connectivity/table_disentangle_demog_xfirm.tex / .csv

Usage:
  ~/.conda/envs/venv_python312/bin/python Programs/layer_connectivity/12c_make_table_disentangle_xfirm.py
"""
...
# ── Key difference from 12_make_table_disentangle.py ─────────────────────────
LAYER_SECTION = "layer_cross_firm"
FIRM_SECTION  = "firm_cross_firm"
...
def build_latex(layers_subset: list, caption: str, label: str) -> str:
    ...
    for pi, layer in enumerate(layers_subset):
        ...
        # Per-panel regressor checkmark rows
    ...
    spec_rows = [
        ...
        ("Microregion $\\times$ Year FE", [r"\checkmark"] * (max_n_lv + 1)),
        ("Industry $\\times$ Year FE",    [r"\checkmark"] * (max_n_lv + 1)),
        ("Mode $\\times$ Year FE",        [r"\checkmark"] * (max_n_lv + 1)),
    ]
    ...
    lines.append(
        ...
        r"(cross-layer or own-layer as shown) and absorb microregion$\times$year, "
        r"industry$\times$year, and mode$\times$year fixed effects. "
        r"Firm-level regressions collapse to firm$\times$year and absorb microregion$\times$year, "
        r"industry$\times$year, and mode$\times$year fixed effects. "
...
CAPTIONS = {
    "demog": "Disentangling layer spillover effects --- education and demographics (cross-firm FE)",
}
LABELS = {
    "demog": "tab:disentangle_demog_xfirm",
}

"""
Assemble horse race table from 01d_horse_race_occ2c.do output.

All four occ4 connectivity variables appear simultaneously in each regression.

Column layout (10 columns):
  Log Dec. wage:   1_mgr | 23_high | 4_bur | 5p_low | Firm
  Log employment:  1_mgr | 23_high | 4_bur | 5p_low | Firm

Row groups (one per regressor):
  c_1_mgr  × Post + SE  |  Pre + SE  |  Pre-F p-value
  c_23_high × Post + SE  |  Pre + SE  |  Pre-F p-value
  c_4_bur  × Post + SE  |  Pre + SE  |  Pre-F p-value
  c_5p_low × Post + SE  |  Pre + SE  |  Pre-F p-value
  Observations / Firms

Output:
  Tables/layer_connectivity/tex_tables/table_horse_race_occ2c.tex
  Tables/layer_connectivity/table_horse_race_occ2c.csv
...
CSV_IN     = TABLES / "results_horse_race_occ2c.csv"

LAYER_VALS = ["upper_skill", "low_skill"]

LAYER_LABELS = {
    "upper_skill": "Upper skill",
    "low_skill":   "Low skill",
}

REGRESSOR_LABELS = {
    "upper_skill": r"$c_{\text{upper}}$",
    "low_skill":   r"$c_{\text{low}}$",
}
...
ALL_COLS = WAGE_COLS + EMP_COLS  # 10 columns total
...
def build_latex(data: dict, caption: str, label: str) -> str:
    ...
    lines.append(
        r" & \multicolumn{3}{c}{Log Dec.\ wage}"
        r" & \multicolumn{3}{c}{Log employment} \\"
    )
    ...
    for lv_r in LAYER_VALS:
        ...
        # Add spacing after each regressor block except the last
    ...
    # Spec checkmarks
    lines.append(r"\midrule")
    # layer cols: first 4 wage + first 4 emp = cols 1-4, 6-9
    # firm cols: col 5 and 10
...
tex_out = TEX_TABLES / "table_horse_race_occ2c.tex"
csv_out = TABLES     / "table_horse_race_occ2c.csv"

********************************************************************************
* UNION SPILLOVERS — HORSE RACE: c_high_skill vs c_low_skill simultaneously (occ2)
...
log using "$logs/layer_connectivity/04_horse_race/horse_race_occ2_`d'_`t'.log", replace text
...
use "$layer_data/firm_layer_outcomes_occ2.dta", clear
keep if year >= 2009
...
foreach lv in high_skill low_skill {
	cap drop c_`lv'_o
	cap drop c_`lv'
...
********************************************************************************
* SECTION 5: LAYER-LEVEL REGRESSIONS (horse race — both c_high_skill and c_low_skill)
...
foreach outcome in lr_remdezr_layer l_layer_emp {
	...
	* occ2 sample is smaller than edu2 — keep FE parsimonious to avoid r(2001)
	...
	foreach lv_o in high_skill low_skill {
		...
		* ── Post-treatment DiD — both regressors simultaneously ──────────────
		reghdfe `outcome' c.c_high_skill##i.treat_year c.c_low_skill##i.treat_year ///
		...
		* c_high_skill main
		local b_no_hs_main  = _b[1.treat_year#c.c_high_skill]
		local se_no_hs_main = _se[1.treat_year#c.c_high_skill]
		...
		* ── Pre-period placebo ───────────────────────────────────────────────
		reghdfe `outcome' c.c_high_skill##i.placebo_year c.c_low_skill##i.placebo_year ///
			if treat_ultra==0 & in_balanced_panel==1 & year<=2011 & layer_id=="`lv_o'", ///
			absorb(i.firm_layer_id i.year `extra') vce(cluster identificad)

		* c_high_skill pre
		local b_no_hs_pre  = _b[1.placebo_year#c.c_high_skill]
		local se_no_hs_pre = _se[1.placebo_year#c.c_high_skill]
		...
		* ── Event study + pre-trend F-tests ─────────────────────────────────
		reghdfe `outcome' c.c_high_skill##ib2011.year c.c_low_skill##ib2011.year ///
			if treat_ultra==0 & in_balanced_panel==1 & layer_id=="`lv_o'", ///
			absorb(i.firm_layer_id i.year `extra') vce(cluster identificad)

		capture testparm c.c_high_skill#i(2009 2010).year
		...
		* ── Write to CSV ─────────────────────────────────────────────────────
		...
		file write `fh' `""horse_race_edu2";"layer_firm_year";"`outcome_key'";"c_high_skill_main";"'    %9.4f (`b_no_hs_main')  `"`st_no_hs_main'"' _n
		file write `fh' `""horse_race_edu2";"layer_firm_year";"`outcome_key'";"c_high_skill_main_se";"' %9.4f (`se_no_hs_main') `"""'              _n
		...
		file write `fh' `""horse_race_edu2";"layer_firm_year";"`outcome_key'";"c_high_skill_pre";"'     %9.4f (`b_no_hs_pre')   `"`st_no_hs_pre'"' _n
		file write `fh' `""horse_race_edu2";"layer_firm_year";"`outcome_key'";"c_high_skill_pre_se";"'  %9.4f (`se_no_hs_pre')  `"""'              _n
		...
		file write `fh' `""horse_race_edu2";"layer_firm_year";"`outcome_key'";"c_high_skill_pre_ftest";"'  %9.4f (`pf_no_hs')   `"""'              _n
...
********************************************************************************
* SECTION 6: FIRM-LEVEL REGRESSIONS (horse race — both c_high_skill and c_low_skill)
********************************************************************************

preserve

	* Collapse to firm×year — c_* are firm-constant so (first) is exact
	collapse (first) c_high_skill c_low_skill treat_ultra in_balanced_panel ///
		industry1 mode_base_month microregion, ///
		by(identificad year)
	...
	* Totalflows quartile bins
	...
	foreach outcome in lr_remdezr_w l_firm_emp {
		...
		* ── Write to CSV ─────────────────────────────────────────────────────
		tempname fh
		file open  `fh' using "`csv_out'", write append
		file write `fh' `""horse_race_edu2";"firm_firm_year";"`outcome'";"c_high_skill_main";"'    %9.4f (`b_no_hs_main')  `"`st_no_hs_main'"' _n
		file write `fh' `""horse_race_edu2";"firm_firm_year";"`outcome'";"c_high_skill_main_se";"' %9.4f (`se_no_hs_main') `"""'              _n
		...
		file write `fh' `""horse_race_edu2";"firm_firm_year";"`outcome'";"c_high_skill_pre";"'     %9.4f (`b_no_hs_pre')   `"`st_no_hs_pre'"' _n
		file write `fh' `""horse_race_edu2";"firm_firm_year";"`outcome'";"c_high_skill_pre_se";"'  %9.4f (`se_no_hs_pre')  `"""'              _n
		...
		file write `fh' `""horse_race_edu2";"firm_firm_year";"`outcome'";"c_high_skill_pre_ftest";"'  %9.4f (`pf_no_hs')   `"""'              _n

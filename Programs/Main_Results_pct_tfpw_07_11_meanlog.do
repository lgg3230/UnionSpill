...
log using "$logs/FinalResults_pct_tfpw_07_11_`d'_`t'.log", replace text
...
* MEAN-OF-LOG VARIANT (2026-08-03).
* The headline script overwrites l_firm_emp_pre (= mean of l_firm_emp over
* 2009-2011, built in the loop above) with ln(mean(firm_emp)).  Those two are
* different statistics, not a monotone transform of one another, so they induce
* different quartile bins and hence different absorbed fixed effects.  This
* variant keeps the mean-of-log definition, matching the convention used by the
* layer/within-firm pipelines (mkprebin) and by cba_value.
* Original three lines retained below, commented, for diffability:
*   cap drop l_firm_emp_pre
*   gen double l_firm_emp_pre = ln(firm_emp_pre)
label var l_firm_emp_pre "Log pre-treatment firm employment (mean of log)"
...
foreach panel in A B C {
	capture erase "$tables/results_direct_panel`panel'_tfpw_07_11_pct.csv"
	tempname fh
	file open `fh' using "$tables/results_direct_panel`panel'_tfpw_07_11_pct.csv", write replace
	file write `fh' "spec,section,outcome,row_type,value" _n
	file close `fh'
}

capture erase "$tables/results_spill_tfpw_07_11_pct.csv"
tempname fh
file open `fh' using "$tables/results_spill_tfpw_07_11_pct.csv", write replace
file write `fh' "spec,section,outcome,row_type,value" _n
file close `fh'
...
foreach panel in A B C {
	...
	local csv_out "$tables/results_direct_panel`panel'_tfpw_07_11_pct.csv"
...
local csv_spill "$tables/results_spill_tfpw_07_11_pct.csv"

...
AUTHOR: LUIS GOMES
PROGRAM: COMBINED COEFFICIENT PLOTS - GRAVITY 6YR + PRETREATMENT 6YR

Creates a single 2x2 figure comparing gravity (6yr) and pretreatment (6yr, 3+3 split):
  - Top row: Proximity measures (left=gravity 6yr, right=pretreat 6yr)
  - Bottom row: Dummy variables (left=gravity 6yr, right=pretreat 6yr)

Key features:
  - Top-aligned: gravity (9 vars) aligns with top 9 of pretreat (10 vars)
...
# 6yr gravity regression coefficients
GRAVITY_6YR_UNIV_FILE = RAIS_AUX / "bilateral_gravity_6yr_coefficients_univariate.csv"
GRAVITY_6YR_MULTI_FILE = RAIS_AUX / "bilateral_gravity_6yr_coefficients_multivariate.csv"

# 6yr pretreatment regression coefficients (3+3 split: early -> late)
PRETREAT_6YR_UNIV_FILE = RAIS_AUX / "bilateral_pretreatment_6yr_coefficients_univariate.csv"
PRETREAT_6YR_MULTI_FILE = RAIS_AUX / "bilateral_pretreatment_6yr_coefficients_multivariate.csv"

# Output
OUTPUT_FILE = GRAPHS / "coefplot_bilateral_6yr.pdf"
...
VAR_LABELS = {
    ...
    "z_bilateral_conn_early_6yr": "Early Conn.\n(2005\u20132008)",
    # Dummies
...
# Gravity 6yr variables (9 proximity, 4 dummies)
...
# Pretreat 6yr variables (10 proximity including early connectivity 6yr, 4 dummies)
PRETREAT_PROXIMITY_VARS = [
    "z_bilateral_conn_early_6yr",
    "z_cep_proximity",
    "z_turnover_proximity",
    "z_size_proximity",
    "z_wage_proximity",
...
def load_gravity_data():
    """Load gravity 6yr coefficients."""
    univ = pd.read_csv(GRAVITY_6YR_UNIV_FILE)
    multi = pd.read_csv(GRAVITY_6YR_MULTI_FILE)
    return pd.concat([univ, multi], ignore_index=True)


def load_pretreat_data():
    """Load pretreatment 6yr coefficients."""
    univ = pd.read_csv(PRETREAT_6YR_UNIV_FILE)
    multi = pd.read_csv(PRETREAT_6YR_MULTI_FILE)
    return pd.concat([univ, multi], ignore_index=True)
...
def plot_coefficients(ax, df, var_list, y_offset=0, max_y_vars=None, forced_order=None):
    """
...
def main():
    print("=" * 70)
    print("CREATING 6YR COMBINED COEFFICIENT PLOT (GRAVITY + PRETREAT)")
    print("=" * 70)
    ...
    print(f"  Gravity 6yr: {len(gravity_df)} rows")
    print(f"  Pretreat 6yr: {len(pretreat_df)} rows")
    ...
    pretreat_order = ["z_bilateral_conn_early_6yr"] + list(grav_order)
    ...
    # =========================================================================
    fig.text(0.28, 0.99, "(A) Dep. Var.: Connectivity (6yr)",
             fontsize=15, fontweight="bold", fontvariant="small-caps",
             ha="center", va="top")
    fig.text(0.72, 0.99, "(B) Dep. Var.: Late Connectivity (6yr)",
             fontsize=15, fontweight="bold", fontvariant="small-caps",
             ha="center", va="top")
